...
def multi_byte_sub(ls, rs):
    if len(ls) < len(rs):
        print(
            "Error in multi_byte_sub. Subtracting a smaller byte range to a larger byte range."
        )
        return [0]
    rs_mod = rs
    while len(ls) > len(rs_mod):
...
class message:
    ...
    def change_message(self, original_bytes):
        ...
        if delta_text_boxes < 0:
            absolute_pointer = multi_byte_sub(
                original_bytes[28:32], [delta_text_boxes * -4]
            )
        else:
            absolute_pointer = multi_byte_add(
                original_bytes[28:32], [delta_text_boxes * 4]
            )
        ...
        for p in self.relative_pointers:
            if p != 0:
                retbytes.extend(
                    multi_byte_add([p + 4], absolute_pointer)
                )  # +4 for some reason fixes it??? I think it's because the first 4 bytes is displayed for what I thought was the header and it does something that you can't normally see.
            else:
                retbytes.extend(multi_byte_add([p], absolute_pointer))
        # keep message header
        retbytes.extend(
            original_bytes[
                28 + (original_text_box_count * 4) : 40 + (original_text_box_count * 4)
            ]
        )
        # write shit in
        for b in self.bytes:
        ...
        if len(retbytes) > len(original_bytes):
            print(
                "BIG WARNING: New byte count exceeds old byte count. This will break something else."
            )
        return retbytes
        ...
        while index < len(processed_str):
            ...
            if ch == "\n":
                ...
                if ch == "^" and (
                    ...
                    print(
                        "WARNING: Unknown escape character: '", esc_ch, "'. Ignoring."
                    )
