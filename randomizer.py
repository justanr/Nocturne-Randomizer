...
class Randomizer:
    ...
    def randomize_skill_mutations(self):
        ...
        slots_left = 55
        new_mutations = {}
        while slots_left > 0 and mutations: #number of mutations should never reach 0, hopefully we always run out of slots
    ...
    def randomize_skills(self, new_demon, force_skills=None, skill_scaling = False):
        ...
        for i in range(total_skills):
            ...
            if chosen_skill.skill_type == 1 and len(new_battle_skills) < 8 and chosen_skill.name not in ['Analyze', 'Trafuri', 'Beckon Call', 'Riberama', 'Lightoma', 'Liftoma', 'Estoma']:
                if (chosen_skill.level <= new_demon.level and chosen_skill.level >= min(new_demon.level - 20, new_demon.level / 2)) or not skill_scaling:
        ...
        if DEV_CHEATS:
            new_battle_skills = [0x33, 0x2D, 0x2B, 0x30, 0xD4] #Recarmdra, Eternal Rest, and 3 status cures

        return [new_skills, new_battle_skills]


    def rebalance_demon(self, old_demon, new_level, stats=None, new_hp=-1, new_mp=-1, new_exp=-1, new_macca=-1, exp_mod=1, stat_mod=1, macca_mod=1):
    ...
    def rebalance_demon_stat_copy(self, old_demon, new_level, new_hp=-1, new_mp=-1, new_exp=-1, new_macca=-1, exp_mod=1, stats=[1, 1, 1, 1, 1], macca_mod=1):
        new_demon = copy.copy(old_demon)
        level_mod = new_level / old_demon.level
        new_demon.level = int(new_level)
    ...
    def randomize_demons(self, demon_map, generated_demons, exp_mod=1, enemy_skill_scaling = False, macca_mod=1):
        ...
        for old_demon in demon_pool:
            ...
            new_demon = self.rebalance_demon(new_demon, old_demon.level, stats=old_demon.stats, new_exp=old_demon.exp_drop, new_macca=old_demon.macca_drop, exp_mod=exp_mod, macca_mod=macca_mod)
            ...
            if new_demon.name == 'Pixie':
                if DEV_CHEATS:
                    new_demon.skills, new_demon.battle_skills = self.randomize_skills(new_demon, [73, 269]) #Estoma and showtime
                else:
    ...
    def randomize_magatamas(self):
        ...
        for old_magatama in nocturne.all_magatamas.values():
            ...
            if new_magatama.name != "Marogareh": #Ensure tutorial is beatable on hard mode
        ...
        for check in world.get_checks():
            ...
            if old_boss is not new_boss:
                ...
                if new_boss_demon.name not in self.always_goes_first:
                    if new_boss_demon.name in self.never_goes_first or not self.config_settings.bosses_go_first_more_often or old_boss_demon.name not in self.go_first_checks:
                ...
                balanced_demon = self.rebalance_demon_stat_copy(new_boss_demon, new_level, new_hp=new_hp, new_mp=new_mp, new_exp=new_exp, new_macca=new_macca, exp_mod=self.config_settings.exp_modifier, stats=old_boss_demon.stats, macca_mod=self.config_settings.macca_modifier)
            else:
            ...
            if extras and (old_boss is not new_boss or DEV_CHEATS):
                ...
                for d in extras:
                    ...
                    if new_boss_demon.name in ["Noah 1st Form (Boss)", 'Kagutsuchi 1st Form (Boss)', 'Ahriman 1st Form (Boss)', 'Specter 1 (Boss)']:
                        d = self.rebalance_demon_stat_copy(nocturne.lookup_demon(d), new_level, new_hp=new_hp, new_mp=new_mp, exp_mod=exp_mod, stats=old_boss_demon.stats, macca_mod=self.config_settings.macca_modifier)
                    else:
                        d = self.rebalance_demon(nocturne.lookup_demon(d), new_level, new_hp=new_hp, new_mp=new_mp, exp_mod=exp_mod, stat_mod=stat_mod, macca_mod=self.config_settings.macca_modifier)
    ...
    def run(self):
        ...
        try:
           self.config_settings, self.input_iso_path, self.text_seed, self.settings_seed, self.config_export_to_host_fs = gui.create_gui(self.config_settings)
        except OSError:
            return
        ...
        new_demons = self.randomize_demons(demon_map, demon_generator.demons, exp_mod=self.config_settings.exp_modifier, enemy_skill_scaling = self.config_settings.enemy_skill_scaling, macca_mod=self.config_settings.macca_modifier)
