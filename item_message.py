...
def changeItemMessageByIndex(message_script, message_obj, index):
    ...
    message_script.names.names_pointers = [x+delta_bytes for x in message_script.names.names_pointers]
...
#12:16 is 0's
msg_script.rolling_pointer = bbtoi(byte_array[c_off+16:c_off+20]) #pointer to gibberish section
msg_script.rolling_size = bbtoi(byte_array[c_off+20:c_off+24]) #size of gibberish section
...
msg_script.rolling_offsets = byte_array[m_off+r_p-0x20:m_off+r_p+r_s-0x20] #for some reason the pointer offset is before the message header instead of after like the other instances?
...
black_key_msg = nass.message("^.^iThe key for the^nBlack Amala Temple.^n^0","item_91",False)
white_key_msg = nass.message("^.^iThe key for the^nWhite Amala Temple.^n^0","item_92",False)
red_key_msg = nass.message("^.^iThe key for the^nRed Amala Temple.^n^n^0","item_93",False)
apocalypse_msg = nass.message("^.^iA stone that attracts^nthe 4 horsemen.^n^0","item_94",False)
goblet_msg = nass.message("^.^iA mother skeleton's^nfavorite cup.^n^0","item_95",False)
eggplant_msg = nass.message("^.^iThe missing piece for^na suspicious ritual.^nIt is very plump^nwith excellent girth.^n^0","item_96",False)
ikebukuro_msg = nass.message("^.^iThe key to entering^nIkebukuro tunnel^n^0","item_97",False)
...
if len(piped_bytes) > 0x3970:
    print("Exporting item messages failed. New message script size is",str(0x3970-len(piped_bytes)),"too large")
else:
