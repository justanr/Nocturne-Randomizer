...
def create_gui(config_settings):
    ...
    def import_iso_path():
        file_path = filedialog.askopenfilename(
            title="Select the path to your iso",
    ...
    def apply_pasted_settings():
        confirmation = tk.messagebox.askyesno(
            title="confirmation", message="Confirm overwriting settings?"
        )
        if not confirmation:
            return
    ...
    pathLabel = tk.Label(
        persistent_frame_top_left,
        text="Please input the path to your SMT3 Nocturne ISO file below",
    )
    pathLabel.pack()
    ...
    seedLabel = tk.Label(
        persistent_frame_bottom_left,
        text="Please input your desired seed value below (blank for random seed)",
    )
    seedLabel.pack()

    seedEntry = tk.Entry(
        persistent_frame_bottom_left, fg="black", bg="#598074", width=50
    )
    seedEntry.pack()

    settingsStringEntry = tk.Entry(
        persistent_frame_top_right, fg="black", bg="#598074", width=50
    )
    settingsStringEntry.pack(side=tk.BOTTOM)

    settingsStringLabel = tk.Label(
        persistent_frame_top_right, text="You may paste in a settings string here"
    )
    ...
    listFlags = tk.Listbox(
        page_1_frame_top,
        selectmode="multiple",
        width=100,
        exportselection=False,
        selectbackground="#598074",
    )
    ...
    listFlags.insert(
        3,
        "Tweak inheritance so that all skills inherit equally regaless of rank or body parts.",
    )
    ...
    listFlags.insert(
        7, "balance skills of enemies based on level (doesn't affect your demons)."
    )
    ...
    listPixie.insert(
        1,
        "Normalize starting Pixie's level between 40 and 60 for a balanced early game",
    )
    listPixie.insert(
        2, "Keep starting Pixie level 2 - not recommended for newer players"
    )
    ...
    listDifficulty = tk.Listbox(
        page_2_frame_right,
        selectmode="multiple",
        width=50,
        exportselection=False,
        selectbackground="#f7333d",
    )
    ...
    def create_settings_string():
        ...
        generalNumber1 = sum(
            [b << i for i, b in enumerate(generalSelection[:4])]
        )  # 0-15
        generalNumber2 = sum(
            [b << i for i, b in enumerate(generalSelection[4:])]
        )  # 0-31
        ...
        musicPixieMutateNumber = (
            16 * listMusic.curselection()[0]
            + 4 * listPixie.curselection()[0]
            + listSkillMutation.curselection()[0]
        )  # 0-63
        musicPixieMutateString = demonNames[
            len(demonNames) - 1 - musicPixieMutateNumber
        ]
        ...
        difficultyNumber = sum(
            [b << i for i, b in enumerate(difficultySelection)]
        )  # 0-63
        difficultyString = demonNames[difficultyNumber]
        reasonCutsceneNumber = (
            4 * listReason.curselection()[0] + listCutscene.curselection()[0]
        )  # 0-18
        reasonCutsceneString = magatamaNames[
            len(magatamaNames) - 1 - reasonCutsceneNumber
        ]
        return (
            generalString1
            + "_"
            + generalString2
            + "_"
            + musicPixieMutateString
            + "_"
            + logicString
            + "_"
            + difficultyString
            + "_"
            + reasonCutsceneString
        )
        ...
        try:
            ...
            if verify_string != settings_string:
                tk.messagebox.showwarning(
                    title="warning",
                    message="Warning: Loss of fidelity in settings string. Please double check your settings",
                )
        except Exception:
            tk.messagebox.showerror(
                title="error",
                message="Error parsing settings string. Changes were not applied",
            )
...
def create_config_file(configur):
    ...
    configur["Settings"] = {
        ...
        "MagicPierce": False,
        "StockHealing": False,
        "RemoveHardmodePrices": False,
        "FixInheritance": False,
        "VanillaTok": False,
        "RandomMusic": False,
        "CheckBasedMusic": False,
        "BalancePixie": False,
        "LowLevelPixie": False,
        "EnemySkillScaling": False,
        "FightLucifer": False,
        "MenorahGroups": False,
        "BetterMutationsStandard": False,
        "BetterMutationsRandom": False,
        "BetterMutationsUnique": False,
        "Yosuga": False,
        "Shijima": False,
        "Musubi": False,
        "NoLoaProgression": False,
        "VanillaPyramidion": False,
        "OpenYurakucho": False,
        "OpenIkebukuro": False,
        "LowBossSafety": False,
        "HighBossSafety": False,
        "BossesGoFirstMoreOften": False,
        "NoLevelSafety": False,
        "NoMagatamaSafety": False,
        "ShortestCutscenes": False,
        "LongestCutscenes": False,
        "LowLevel": False,
    }
