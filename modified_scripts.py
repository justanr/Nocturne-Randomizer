...
class Script_Modifier:
    ...
    def get_reward_str(self, check_name, world):
        ...
        if world.checks[check_name].boss.reward:
            reward_str += (
                "^n"
                + custom_vals.MAGATAMA_REWARD_MSG[
                    world.checks[check_name].boss.reward.name
                ]
            )
        if world.checks[check_name].flag_rewards:
            for flag_reward in world.checks[check_name].flag_rewards:
                if flag_reward.flag_id in custom_vals.FLAG_REWARD_MSG:
                    reward_str += (
                        "^n" + custom_vals.FLAG_REWARD_MSG[flag_reward.flag_id]
                    )
                else:
                    print(
                        "Warning: In get_reward_str(). No reward string found for flag",
                        hex(flag_reward.flag_id),
                    )
        return reward_str
    ...
    def get_flag_reward_insts(self, check_name, world):
        ...
        for flag in world.checks[check_name].flag_rewards:
            ...
            if flag.additional_ids:
                for extra_id in flag.additional_ids:
                    ...
                    ret_insts.append(inst("COMM", 8))
        if (
            world.checks[check_name].boss.reward is not None
        ):  # Just move all magatama rewards to events because those are less glitchy
        ...
        ret_insts.extend(
            [inst("PUSHIS", 20), inst("COMM", 0xE)]
        )  # small pause so you can read through skipping
        return ret_insts
    ...
    def get_flag_reward_location_string(self, flag_id, world):
        ...
        print(
            "Warning: In get_flag_reward_location_string(), flag",
            hex(flag_id),
            "not found.",
        )
        return ""
    ...
    def get_checks_boss_id(self, check_name, world, index=0):
        ...
        print(
            "Error: a field boss model does not exist and will show a generic replacement"
        )
        return 122  # Mothman!
    ...
    def insert_callback(
        self, field_string, location_insert, fun_name_insert, overwrite_warning=True
    ):
        if len(fun_name_insert) > 15:
            print(
                "ERROR: In insert_callback().",
                fun_name_insert,
        ...
        if wap_file[location_insert] != 0 and overwrite_warning:
            print(
                "WARNING: Callback insertion of", fun_name_insert, "overwriting data."
            )
        wap_file = (
            wap_file[:location_insert]
            + bytes([2])
            + bytes(assembler.ctobb(fun_name_insert, 15))
            + wap_file[location_insert + 16 :]
        )
    ...
    def run(self, world=None, config_settings=None):
        ...
        e601_insts = [
            ...
            inst(
                "PUSHIS", 0x520
            ),  # Actual Ginza Underpass splash (What is actually 0x510?)
            inst("COMM", 8),
            inst(
                "PUSHIS", 0x526
            ),  # Turns on encounters in Ginza Underpass. Combine with open underpass.
            inst("COMM", 8),
            ...
            inst(
                "PUSHIS", 0x650
            ),  # Sisters Talk at entrance (Consider to keep on, but change their models to the randomized boss)
            inst("COMM", 8),
        ...
        if config_settings.open_ikebukuro:
            e601_bonus_insts += [inst("PUSHIS", 0x3F7), inst("COMM", 0x8)]

        if config_settings.open_yurakucho:
            e601_bonus_insts += [inst("PUSHIS", 0x56), inst("COMM", 0x8)]

        e601_insts = e601_insts[:-21] + e601_bonus_insts + e601_insts[-21:]

        e601_obj.changeProcByIndex(
            e601_insts, [], 0
        )  # empty list is relative branch labels
        ...
        f015_pixie_insts, f015_pixie_labels = f015_obj.getProcInstructionsLabelsByIndex(
            f015_pixie_proc
        )
        precut = 32
        postcut = 356
        ...
        for l in f015_pixie_labels:
            if l.label_offset > precut:
                ...
                if l.label_offset < 1:
                    l.label_offset = 1
        f015_pixie_insts = (
            f015_pixie_insts[:precut]
            + f015_pixie_insts[postcut:precut2]
            + f015_pixie_insts[postcut2:]
        )
        ...
        f015_012_start_insts = [
            ...
            inst(
                "PUSHREG"
            ),  # Push the result of previous operation as a parameter, in this case, 0 == flagcheck(7)
        ...
        f015_012_start_labels = [
            assembler.label(
                "PRETA_FIGHT_DONE", 20
            ),  # both needs to be changed if above procedure is shifted
            assembler.label("GATE_PASS_OBTAINED", 19),  # given number is line number
        ]
        f015_obj.changeProcByIndex(
            f015_012_start_insts, f015_012_start_labels, tri_preta_room_index
        )

        # Forneus
        forneus_room_index = f015_obj.getProcIndexByLabel(
            "002_start"
        )  # index 18 / 0x12
        ...
        if config_settings.shortest_cutscenes:  # Skip Forneus cutscene
            ...
            f015_002_start_labels = [assembler.label("FORNEUS_DEAD", 24)]
            f015_obj.changeProcByIndex(
                f015_002_start_insts, f015_002_start_labels, forneus_room_index
            )
        else:  # Watch Forneus cutscene, set model and message box
            f015_002_start_insts, f015_002_start_labels = (
                f015_obj.getProcInstructionsLabelsByIndex(forneus_room_index)
            )
            f015_002_start_insts[29] = inst(
                "PUSHIS", self.get_checks_boss_id("Forneus", world)
            )
            f015_002_start_insts[44] = inst(
                "PUSHIS", 0x4
            )  # Magic anim instead of unique skill
            f015_obj.changeProcByIndex(
                f015_002_start_insts, f015_002_start_labels, forneus_room_index
            )
            f015_forneus_name_id = f015_obj.sections[3].messages[0x31].name_id
            f015_forneus_message = assembler.message(
                "Hey, punk! I've never seen you^naround, and I don't like your face!^n^xYou're trying to get by^nwithout paying respect to me,^nThe Almighty "
                + self.get_checks_boss_name("Forneus", world, immersive=True)
                + "!?^xYou wanna die, don't ya!?.",
                "boss_01",
            )
            f015_forneus_message.name_id = f015_forneus_name_id
            f015_obj.changeMessageByIndex(f015_forneus_message, 0x31)
            f015_obj.changeNameByLookup(
                "Forneus", self.get_checks_boss_name("Forneus", world, immersive=True)
            )

        f015_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Forneus", world), "FORNEUS_REWARD"),
            0x5B,
        )

        # 000_dh_plus - flag insertion
        f015_forneus_reward_proc = f015_obj.getProcIndexByLabel("000_dh_plus")
        f015_forneus_reward_insts, f015_forneus_reward_labels = (
            f015_obj.getProcInstructionsLabelsByIndex(f015_forneus_reward_proc)
        )  # Has no labels.
        f015_forneus_reward_insts = (
            f015_forneus_reward_insts[:-1]
            + self.get_flag_reward_insts("Forneus", world)
            + [inst("END")]
        )
        f015_obj.changeProcByIndex(
            f015_forneus_reward_insts, [], f015_forneus_reward_proc
        )

        f015_obj.changeMessageByIndex(
            assembler.message(
                "What!? You're going to defeat^n^r"
                + self.get_checks_boss_name("Forneus", world)
                + "^p!?",
                "F015_SINEN19_01",
            ),
            0xF1,
        ...
        f015_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("Black Rider", world)
                + "^p.",
                "FIRE_YURE",
            ),
            0x6E,
        ...
        f015_br_insts, f015_br_labels = f015_obj.getProcInstructionsLabelsByIndex(
            f015_br_proc
        )
        f015_br_insts[4] = inst(
            "PUSHIS", 0x3F4
        )  # Change rider trigger check from 7b8 to key item
        f015_br_insts[7] = inst(
            "PUSHIS", 0x3F4
        )  # "Remove" story trigger check. (Yuko in Obelisk cutscene)
        ...
        if not config_settings.longest_cutscenes:
            ...
            f015_14_insts, f015_14_labels = f015_obj.getProcInstructionsLabelsByIndex(
                f015_14_proc
            )
        ...
        f015_brider_rwms_index = f015_obj.appendMessage(
            self.get_reward_str("Black Rider", world), "BR_REWARD"
        )
        f015_br_rwms_insts = (
            [
        ...
        e745_10_insts, e745_10_labels = e745_obj.getProcInstructionsLabelsByIndex(
            e745_10_proc
        )
        ...
        e745_main_insts, e745_main_labels = e745_obj.getProcInstructionsLabelsByIndex(
            e745_main_proc
        )
        e745_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("Black Rider", world)
        )
        e745_main_insts[55] = inst("PUSHIS", 0x6)
        e745_main_insts[56] = inst("COMM", 0x15)
        if (
            config_settings.menorah_groups
        ):  # Keep menorah flag and textbox if the vanilla flag is on
        ...
        e745_br_message = assembler.message(
            "As long as thou seekest^nthe candelabra...^nthou art mine enemy...^xI... the "
            + self.get_checks_boss_name("Black Rider", world, immersive=True)
            + "...^nshall judge thee...",
            "MSG_002",
        )
        e745_br_message.name_id = e745_br_name_id
        e745_obj.changeMessageByIndex(e745_br_message, 0x1)
        e745_obj.changeNameByLookup(
            "Black Rider",
            self.get_checks_boss_name("Black Rider", world, immersive=True),
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e745"], BytesIO(bytes(e745_obj.toBytes()))
        )
        ...
        f017_01_insts, f017_01_labels = f017_obj.getProcInstructionsLabelsByIndex(
            f017_01_proc
        )
        ...
        diff = postcut - precut
        f017_01_insts = (
            f017_01_insts[:precut] + f017_01_insert_insts + f017_01_insts[postcut:]
        )
        ...
        f017_09_insts, f017_09_labels = f017_obj.getProcInstructionsLabelsByIndex(
            f017_09_proc
        )
        if not config_settings.longest_cutscenes:
            precut = 10
            ...
            f017_09_insts = (
                f017_09_insts[:precut] + f017_09_insert_insts + f017_09_insts[postcut:]
            )
            ...
            f017_09_insts[10] = inst(
                "PUSHIS", self.get_checks_boss_id("Mara", world, index=1)
            )
            f017_09_insts[196] = inst("PUSHIS", self.get_checks_boss_id("Mara", world))
            f017_09_insert_insts = self.get_flag_reward_insts("Mara", world)
            f017_09_insts = (
                f017_09_insts[:10] + f017_09_insert_insts + f017_09_insts[10:]
            )
            for l in f017_09_labels:
                if l.label_offset > 10:
                    l.label_offset += len(f017_09_insert_insts)
            f017_baphomet_name_id = f017_obj.sections[3].messages[0x6].name_id
            f017_baphomet_message = assembler.message(
                "............^n^x*chuckle* As I haaaad been saying,^nit's not time yet.^n^x"
                + self.get_checks_boss_name("Mara", world, immersive=True)
                + " is a magnificent demon...^n^xIf we summon him at the wrong^ntime, something baaaad may^nhappen.",
                "BAFO_01",
            )
            f017_baphomet_message.name_id = f017_baphomet_name_id
            f017_obj.changeMessageByIndex(f017_baphomet_message, 0x6)
            f017_baphomet_2_message = assembler.message(
                "*chuckle*^n^xWell! In my rush, I have summoned^n"
                + self.get_checks_boss_name("Mara", world, immersive=True)
                + " the Magnificent in an^nincomplete body of a slime.",
                "BAFO_08",
            )
            ...
            f017_manikin_left_message = assembler.message(
                "Right on! It's payback time!^n^xHurry up and summon that^nmagnificent demon!^n^xSummon the magnificent ^r"
                + self.get_checks_boss_name("Mara", world, immersive=True)
                + "^p!^nThis is going to be sooo sweet!!",
                "MANE_B_01",
            )
            f017_manikin_left_message.name_id = f017_manikin_left_name_id
            f017_obj.changeMessageByIndex(f017_manikin_left_message, 0x5)
            f017_manikin_left_2_message = assembler.message(
                "See?! He thinks so too!^n^xNow, hurry up and summon "
                + self.get_checks_boss_name("Mara", world, immersive=True)
                + "!",
                "MANE_B_03_YES",
            )
            ...
            f017_manikin_left_3_message = assembler.message(
                "That's right, Mr. "
                + self.get_checks_boss_name("Mara", world, immersive=True, index=1)
                + ".^nJust like the guy on the right said!",
                "MANE_B_02",
            )
            ...
            f017_manikin_right_message = assembler.message(
                "You there! You tell him, too!^n^xTell him to summon "
                + self.get_checks_boss_name("Mara", world, immersive=True)
                + " and^nmake our lives a little easier!",
                "MANE_B_03",
            )
            ...
            f017_manikin_right_2_message = assembler.message(
                "Ah! You're back!^n^xAre you here to tell him to hurry up^nand summon "
                + self.get_checks_boss_name("Mara", world, immersive=True)
                + "???",
                "MANE_B_03_2ND",
            )
            ...
            f017_obj.changeNameByLookup(
                "Baphomet",
                self.get_checks_boss_name("Mara", world, immersive=True, index=1),
            )
            f017_obj.changeNameByLookup(
                "Mara", self.get_checks_boss_name("Mara", world, immersive=True)
            )
        f017_obj.changeProcByIndex(f017_09_insts, f017_09_labels, f017_09_proc)
        f017_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Mara", world), "MARA_RWMS"), 0x19
        )

        # Mara hint message
        f017_obj.changeMessageByIndex(
            assembler.message(
                "> A ceremony is being prepared^nto summon ^r"
                + self.get_checks_boss_name("Mara", world)
                + "^p with an eggplant^nfrom ^g"
                + self.get_flag_reward_location_string(0x3F6, world)
                + "^p.",
                "AYASII",
            ),
            0x1,
        ...
        f017_wr_insts, f017_wr_labels = f017_obj.getProcInstructionsLabelsByIndex(
            f017_wr_proc
        )
        f017_wr_insts[4] = inst("PUSHIS", 0x3F4)
        f017_obj.changeProcByIndex(f017_wr_insts, f017_wr_labels, f017_wr_proc)
        f017_wr2_proc = f017_obj.getProcIndexByLabel(
            "003_pixy"
        )  # Proc for both white rider (again) and Pixie
        f017_wr2_insts, f017_wr2_labels = f017_obj.getProcInstructionsLabelsByIndex(
            f017_wr2_proc
        )
        f017_wr2_insts[4] = inst("PUSHIS", 0x3F4)
        f017_wr2_cutoff = (
            87  # Remove Pixie blocking path if you haven't gone to Yoyogi park
        )
        ...
        f017_03_insts = [  # See Daisoujou proc for more detailed comments on this proc
            ...
            inst(
                "PUSHIS", 0x5C0
            ),  # "Story trigger" to enable Riders, which is already set from the start.
            inst("COMM", 7),
            inst("PUSHREG"),
            inst("PUSHIS", 0x3F4),  # Key item to enable Riders
            inst("COMM", 7),
        ...
        if not config_settings.longest_cutscenes:
            ...
            f017_03_1_insts, f017_03_1_labels = (
                f017_obj.getProcInstructionsLabelsByIndex(f017_03_1_proc)
            )
            f017_03_1_insts[4] = inst("PUSHIS", 0x3F4)
            f017_obj.changeProcByIndex(
                f017_03_1_insts, f017_03_1_labels, f017_03_1_proc
            )
        f017_obj.changeProcByIndex(f017_03_2_insts, [], f017_03_2_proc)
        f017_obj.changeProcByIndex(f017_03_3_insts, [], f017_03_3_proc)
        f017_wr_rwms_index = f017_obj.appendMessage(
            self.get_reward_str("White Rider", world), "WR_RWMS"
        )
        ...
        f017_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("White Rider", world)
                + "^p.",
                "FIRE_YURE",
            ),
            0x34,
        ...
        e743_10_insts, e743_10_labels = e743_obj.getProcInstructionsLabelsByIndex(
            e743_10_proc
        )
        ...
        e743_10_insts[131] = inst(
            "PUSHSTR", 267
        )  # Fix camera to show the boss at all times
        ...
        e743_10_insert_insts = [
            ...
            inst(
                "COMM", 0x4A
            ),  # Commands that use the model's integer index and are always run when loading
            inst("PUSHIX", 0x1),
            inst("COMM", 0x21E),
        ]
        e743_10_insts = (
            e743_10_insts[:74]
            + e743_10_insert_insts
            + e743_10_insts[74:189]
            + e743_10_insts[200:216]
            + e743_10_insts[227:]
        )
        ...
        e743_main_insts, e743_main_labels = e743_obj.getProcInstructionsLabelsByIndex(
            e743_main_proc
        )
        e743_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("White Rider", world)
        )
        ...
        e743_obj.changeNameByLookup(
            "White Rider",
            self.get_checks_boss_name("White Rider", world, immersive=True),
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e743"], BytesIO(bytes(e743_obj.toBytes()))
        )
        ...
        e623_insts, e623_labels = e623_obj.getProcInstructionsLabelsByIndex(
            e623_trm_proc
        )
        ...
        # Specter 1 hint
        e623_obj.changeMessageByIndex(
            assembler.message(
                "Help me rid the Amala^nNetwork of ^r"
                + self.get_checks_boss_name("Specter 1", world)
                + "^p.",
                "MSG_TRM_1",
            ),
            0xD,
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e623"], BytesIO(bytes(e623_obj.toBytes()))
        )
        ...
        f018_01_insts, f018_01_labels = f018_obj.getProcInstructionsLabelsByIndex(
            f018_01_room
        )
        f018_01_cutoff = 15
        ...
        f018_02_insts, f018_02_labels = f018_obj.getProcInstructionsLabelsByIndex(
            f018_02_room
        )
        precut = 55
        postcut = 164
        ...
        f018_07_insts, f018_07_labels = f018_obj.getProcInstructionsLabelsByIndex(
            f018_07_room
        )
        precut = 91
        postcut = 272
        ...
        f018_09_insts, f018_09_labels = f018_obj.getProcInstructionsLabelsByIndex(
            f018_09_room
        )
        f018_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Specter 1", world), "SPEC1_REWARD"),
            0x27,
        )
        if not config_settings.longest_cutscenes:
            f018_09_insert_insts = (
                [  # Instructions to be inserted before fighting specter 1
            ...
            diff2 = postcut2 - precut2
            f018_09_insts = (
                f018_09_insts[:precut1]
                + f018_09_insts[postcut1:precut2]
                + f018_09_insert_insts
                + f018_09_insts[postcut2:]
            )
            ...
            f018_09_insts[37] = inst(
                "PUSHIS", self.get_checks_boss_id("Specter 1", world)
            )
            f018_09_insts[182] = inst(
                "PUSHIS", self.get_checks_boss_id("Specter 1", world)
            )
            f018_09_insts[287] = inst(
                "PUSHIS", self.get_checks_boss_id("Specter 1", world)
            )
            f018_09_insts = (
                f018_09_insts[:161]
            ...
            f018_obj.changeNameByLookup(
                "Specter", self.get_checks_boss_name("Specter 1", world, immersive=True)
            )
        ...
        f018_wap = bytearray(
            self.dds3.get_file_from_path(custom_vals.WAP_PATH["f018"]).read()
        )
        ...
        f018_lb.read_lb()
        f018_lb = f018_lb.export_lb(
            {"BF": BytesIO(bytearray(f018_obj.toBytes())), "WAP": BytesIO(f018_wap)}
        )
        ...
        f019_troll_rwms_index = f019_obj.appendMessage(
            self.get_reward_str("Troll", world), "TROLL_REWARD"
        )
        ...
        f019_troll_insts, f019_troll_labels = f019_obj.getProcInstructionsLabelsByIndex(
            f019_troll_proc
        )
        if config_settings.shortest_cutscenes:  # Remove troll cutscene
            ...
            f019_troll_insts = (
                f019_troll_insts[:f019_troll_precut]
                + f019_troll_insts[f019_troll_postcut:]
            )
            ...
            f019_troll_insts[13] = inst(
                "PUSHIS", self.get_checks_boss_id("Troll", world)
            )
            f019_troll_insts[44] = inst(
                "PUSHIS", self.get_checks_boss_id("Troll", world)
            )
        f019_obj.changeProcByIndex(f019_troll_insts, f019_troll_labels, f019_troll_proc)

        f019_obj.changeMessageByIndex(
            assembler.message(
                "> The door was locked by^n^r"
                + self.get_checks_boss_name("Troll", world)
                + "^p.^x> Will you unlock it?",
                "F019_DOOR01a",
            ),
            0x51,
        )
        ...
        f019_rider_flame_insts, f019_rider_flame_labels = (
            f019_obj.getProcInstructionsLabelsByIndex(f019_rider_flame_proc)
        )
        f019_rider_flame_insts[6] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f019_obj.changeProcByIndex(
            f019_rider_flame_insts, f019_rider_flame_labels, f019_rider_flame_proc
        )
        f019_rider_fight_proc = f019_obj.getProcIndexByLabel("001_01eve_01")
        f019_rider_fight_insts, f019_rider_fight_labels = (
            f019_obj.getProcInstructionsLabelsByIndex(f019_rider_fight_proc)
        )
        f019_rider_fight_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f019_obj.changeProcByIndex(
            f019_rider_fight_insts, f019_rider_fight_labels, f019_rider_fight_proc
        )
        f019_rider_fight2_proc = f019_obj.getProcIndexByLabel("001_01eve_02")
        f019_rider_fight2_insts, f019_rider_fight2_labels = (
            f019_obj.getProcInstructionsLabelsByIndex(f019_rider_fight2_proc)
        )
        f019_rider_fight2_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f019_obj.changeProcByIndex(
            f019_rider_fight2_insts, f019_rider_fight2_labels, f019_rider_fight2_proc
        )
        f019_rider_fight3_proc = f019_obj.getProcIndexByLabel(
            "001_01eve_03"
        )  # There are 3 Ginza rider battles because why?
        f019_rider_fight3_insts, f019_rider_fight3_labels = (
            f019_obj.getProcInstructionsLabelsByIndex(f019_rider_fight3_proc)
        )
        f019_rider_fight3_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f019_obj.changeProcByIndex(
            f019_rider_fight3_insts, f019_rider_fight3_labels, f019_rider_fight3_proc
        )
        ...
        f022_013_e1_insts = [
            ...
            inst(
                "PUSHIS", 0x921
            ),  # Gets set, but not sure what it does? Maybe this is the textbox flag.
            inst("COMM", 8),
            inst("PUSHIS", 0x2E4),
            inst("PUSHIS", 0x16),
            inst("PUSHIS", 1),
        ...
        f022_013_e1_labels = [assembler.label("MATADOR_GONE", 21)]
        ...
        f022_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Matador", world), "MATA_REWARD"),
            0x1D,
        )
        f022_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("Matador", world)
                + "^p.",
                "F22_YURE",
            ),
            0x1A,
        )
        f022_rr_hint_msg = f022_obj.appendMessage(
            "You sense the presence of^n^r"
            + self.get_checks_boss_name("Red Rider", world)
            + "^p.",
            "RR_HINT",
        )
        ...
        if not config_settings.longest_cutscenes:
            f022_obj.changeProcByIndex(
                f022_013_e1_insts, f022_013_e1_labels, f022_mata_room
            )

        f022_rr_proc = f022_obj.getProcIndexByLabel("010_r_rider")
        f022_rr_insts, f022_rr_labels = f022_obj.getProcInstructionsLabelsByIndex(
            f022_rr_proc
        )
        f022_rr_insts[4] = inst(
            "PUSHIS", 0x3F4
        )  # Change rider trigger check from 7b8 to key item
        ...
        if not config_settings.longest_cutscenes:
            ...
            f022_10_insts, f022_10_labels = f022_obj.getProcInstructionsLabelsByIndex(
                f022_10_proc
            )
        ...
        f022_rrider_rwms_index = f022_obj.appendMessage(
            self.get_reward_str("Red Rider", world), "RR_REWARD"
        )
        f022_rr_rwms_insts = (
            [
        ...
        e740_10_insts, e740_10_labels = e740_obj.getProcInstructionsLabelsByIndex(
            e740_10_proc
        )
        ...
        e740_main_insts, e740_main_labels = e740_obj.getProcInstructionsLabelsByIndex(
            e740_main_proc
        )
        ...
        e740_matador_message = assembler.message(
            "It is meant for a master^nswordsman...^n^xone who, amidst blood and^napplause, has put an end^nto countless lives...^xThat warrior is I, ^r"
            + self.get_checks_boss_name("Matador", world, immersive=True)
            + "^p.^n^xIt's unfortunate that we have^nno spectators,^n^xbut I believe this will be^nan excellent show regardless,^nas you and I contend for^nthe candelabra.",
            "boss_01",
        )
        e740_matador_message.name_id = e740_matador_name_id
        e740_obj.changeMessageByIndex(e740_matador_message, 0x3)
        e740_obj.changeNameByLookup(
            "Matador", self.get_checks_boss_name("Matador", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e740"], BytesIO(bytes(e740_obj.toBytes()))
        )
        ...
        e744_10_insts, e744_10_labels = e744_obj.getProcInstructionsLabelsByIndex(
            e744_10_proc
        )
        ...
        e744_10_insts = (
            e744_10_insts[:128] + e744_10_insts[139:149] + e744_10_insts[160:]
        )
        ...
        e744_main_insts, e744_main_labels = e744_obj.getProcInstructionsLabelsByIndex(
            e744_main_proc
        )
        e744_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("Red Rider", world)
        )
        ...
        e744_obj.changeNameByLookup(
            "Red Rider", self.get_checks_boss_name("Red Rider", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e744"], BytesIO(bytes(e744_obj.toBytes()))
        )
        ...
        f023_03_insts = [
            ...
            inst(
                "IF", 1
            ),  # If no is selected, go to label 1. Differs from label 0 in that you set 0x753
        ...
        f023_03_room_2 = f023_obj.getProcIndexByLabel(
            "003_01eve_01"
        )  # Completely copy-pasted from the above, but is triggered from a different position. Just call the other one dammit.
        f023_03_2_insts = [
        ...
        f023_daisoujou_rwms_index = f023_obj.appendMessage(
            self.get_reward_str("Daisoujou", world), "DAI_REWARD"
        )

        f023_daisoujou_hint_msg = f023_obj.appendMessage(
            "You sense the presence of^n^r"
            + self.get_checks_boss_name("Daisoujou", world)
            + "^p.",
            "DAI_HINT",
        )
        f023_03_start_proc = f023_obj.getProcIndexByLabel("003_daisojo")
        f023_03_start_insts, f023_03_start_labels = (
            f023_obj.getProcInstructionsLabelsByIndex(f023_03_start_proc)
        )
        f023_03_start_insts[42] = inst("PUSHIS", f023_daisoujou_hint_msg)
        f023_obj.changeProcByIndex(
            f023_03_start_insts, f023_03_start_labels, f023_03_start_proc
        )

        f023_proclen = len(f023_obj.p_lbls().labels)
        f023_daisoujou_rwmspr_insts = (
            [  # reward message proc
        ...
        f023_obj.appendProc(
            f023_daisoujou_rwmspr_insts, [], f023_daisoujou_callback_str
        )
        self.insert_callback("f023", 0x284, f023_daisoujou_callback_str)
        # seek to 0x284 of f023.wap. write 02 then "DAI_RWMSPR"

        f023_obj.changeMessageByIndex(
            assembler.message(
                "I've come here to join the Mantra,^nso that I can become just like^nthe great King, hee ho!^n............^n^xBut, I'm too scared to go in, hee ho!^n^r"
                + self.get_checks_boss_name("Dante 1", world)
                + "^p's on the roof ho.",
                "F023_HIHO",
            ),
            0x6F,
        ...
        f023_rider_flame_insts, f023_rider_flame_labels = (
            f023_obj.getProcInstructionsLabelsByIndex(f023_rider_flame_proc)
        )
        f023_rider_flame_insts[6] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f023_obj.changeProcByIndex(
            f023_rider_flame_insts, f023_rider_flame_labels, f023_rider_flame_proc
        )
        f023_rider_fight_proc = f023_obj.getProcIndexByLabel("006_01eve_02")
        f023_rider_fight_insts, f023_rider_fight_labels = (
            f023_obj.getProcInstructionsLabelsByIndex(f023_rider_fight_proc)
        )
        f023_rider_fight_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f023_obj.changeProcByIndex(
            f023_rider_fight_insts, f023_rider_fight_labels, f023_rider_fight_proc
        )
        ...
        f023_01_dante_labels = [assembler.label("DANTE_FOUGHT", 19)]
        f023_obj.changeProcByIndex(
            f023_01_dante_insts, f023_01_dante_labels, f023_01_dante_proc
        )

        f023_dante_callback_str = "DANTE_CB"
        f023_dante_reward_index = f023_obj.appendMessage(
            self.get_reward_str("Dante 1", world), "DANTE_REWARD"
        )
        ...
        f023_dante_hint_msg = f023_obj.appendMessage(
            "You sense the presence of^n^r"
            + self.get_checks_boss_name("Dante 1", world)
            + "^p.",
            "DANTE1_HINT",
        ...
        f023_01_start_insts, f023_01_start_labels = (
            f023_obj.getProcInstructionsLabelsByIndex(f023_01_start_proc)
        )
        f023_01_start_insts[47] = inst("PUSHIS", f023_dante_hint_msg)
        f023_obj.changeProcByIndex(
            f023_01_start_insts, f023_01_start_labels, f023_01_start_proc
        )
        ...
        e742_10_insts, e742_10_labels = e742_obj.getProcInstructionsLabelsByIndex(
            e742_10_proc
        )
        ...
        e742_main_insts, e742_main_labels = e742_obj.getProcInstructionsLabelsByIndex(
            e742_main_proc
        )
        e742_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("Daisoujou", world)
        )
        ...
        e742_obj.changeNameByLookup(
            "Daisoujou", self.get_checks_boss_name("Daisoujou", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e742"], BytesIO(bytes(e742_obj.toBytes()))
        )
        ...
        f024_thor_gauntlet_msg_index = f024_obj.appendMessage(
            "Do you want to go directly to the Thor gauntlet? It starts with ^r"
            + self.get_checks_boss_name("Orthrus", world)
            + "^p.",
            "THOR_GAUNTLET_MSG",
        )
        f024_thor_gauntlet_msg_no_index = f024_obj.appendMessage(
            "If you would like to do the Thor gauntlet, go to the center room^non the 3rd floor.",
            "THOR_GAUNTLET_MSG_NO",
        )
        ...
        f024_01_insts = [
            ...
            inst(
                "IF", 1
            ),  # Semi-end label. Show the "no" textbox and give control back.
            inst("COMM", 0x61),
            inst("COMM", 2),
            inst("PUSHIS", 0x565),
            inst(
                "COMM", 0x8
            ),  # TODO in here. Close the Thor gauntlet jail if that setting is on.
            inst("PUSHIS", 0x1F4),
            inst("PUSHIS", 0x18),
            inst("PUSHIS", 0x1),
            inst("COMM", 0x97),
        ...
        f024_01_labels = [assembler.label("_01_END", 34), assembler.label("_01_NO", 30)]

        f024_obj.changeProcByIndex(f024_01_insts, f024_01_labels, f024_01_room)

        f024_10_room = f024_obj.getProcIndexByLabel("010_start")
        f024_10_insts, f024_10_labels = f024_obj.getProcInstructionsLabelsByIndex(
            f024_10_room
        )
        ...
        f024_orthrus_rwms = f024_obj.appendMessage(
            self.get_reward_str("Orthrus", world), "ORTHRUS_RWMS"
        )
        ...
        f024_yaksini_rwms = f024_obj.appendMessage(
            self.get_reward_str("Yaksini", world), "YAKSINI_RWMS"
        )
        ...
        # from  726-881
        f024_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Thor 1", world), "THOR_REWARD"), 97
        )
        f024_10_insert_insts_thor_post = (
            [  # double-check the flags here. Dante might not spawn.
                ...
                inst(
                    "PUSHIS", 0x549
                ),  # Dante Flag. 0x100 needs to not be set for this to work correctly.
                inst("COMM", 8),
        ...
        f024_10_insts = (
            f024_10_insts[:precut1]
            + f024_10_insert_insts
            + f024_10_insts[postcut1:precut1p5]
            + f024_10_insert_insts_yaksini
            + f024_10_insts[postcut1p5:precut2]
            + f024_10_insert_insts_thor_pre
            + f024_10_insts[postcut2:precut3]
            + f024_10_insert_insts_thor_post
            + f024_10_insts[postcut3:]
        )
        ...
        f024_obj.changeNameByLookup(
            "Orthrus", self.get_checks_boss_name("Orthrus", world, immersive=True)
        )
        f024_obj.changeNameByLookup(
            "Yaksini", self.get_checks_boss_name("Yaksini", world, immersive=True)
        )
        f024_obj.changeNameByLookup(
            "Thor", self.get_checks_boss_name("Thor 1", world, immersive=True)
        )
        f024_yaksini_name_id = f024_obj.sections[3].messages[0x59].name_id
        f024_yaksini_message = assembler.message(
            "Just beating "
            + self.get_checks_boss_name("Orthrus", world, immersive=True)
            + " isn't going to^noverturn the decision.^n^xYour sentence is death...^nAhh... I want to cut you up right now!^n^xBut, I'll give you some time.^nMake it fun for me.",
            "K_TARA_01",
        )
        ...
        f024_thor_message = assembler.message(
            "I am "
            + self.get_checks_boss_name("Thor 1", world, immersive=True)
            + ".^n^xI commend your ability to fight.^n^xBut...^n^xWill your power work against me?^n^xMy hammer shall be the judge.^n^xI will give you time to prepare.",
            "K_TORU_01",
        )
        ...
        self.dds3.add_new_file(
            custom_vals.LB0_PATH["f024b"], f024_lb
        )  # for some reason there's regular, b and c
        self.dds3.add_new_file(custom_vals.LB0_PATH["f024c"], f024_lb)

        if SCRIPT_DEBUG:
            self.script_debug_out(f024_obj, "f024.bf")

        e661_obj = self.get_script_obj_by_name(
            "e661"
        )  # Remove chiaki cutscene in mantra HQ
        e661_insts = [
            inst("PROC", 0),
            inst("PUSHIS", 0x10),
            inst("PUSHIS", 1),
        ...
        e661_obj.changeMessageByIndex(
            assembler.message("You have no business here.", "MSG_020_1"), 0x0
        )
        e661_obj.changeProcByIndex(e661_insts, [], 0)
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e661"], BytesIO(bytes(e661_obj.toBytes()))
        )
        ...
        f020_01_insts = [inst("PROC", f020_01_room), inst("END")]
        f020_02_room = f020_obj.getProcIndexByLabel("002_start")
        f020_02_insts = [inst("PROC", f020_02_room), inst("END")]
        f020_14_room = f020_obj.getProcIndexByLabel("014_start")
        f020_14_insts = [inst("PROC", f020_14_room), inst("END")]
        no_labels = []
        ...
        f020_08_insts, f020_08_labels = f020_obj.getProcInstructionsLabelsByIndex(
            f020_08_room
        )
        precut = 48
        postcut = 179
        if config_settings.longest_cutscenes:
            ...
            f020_obj.changeNameByLookup(
                "Incubus", world.demons[incubus_replacement].name
            )
        ...
        f020_08_insert_insts_autokilacheck = [
            ...
            inst(
                "PUSHIS", 0x4E3
            ),  # 4e3 set and 6d9 unset. For Koppa / Incubus. (Note: Possibly doesn't work, but doesn't seem to harm things either way. It's also a super edge-case scenario for randomizing kilas in with other stuff.)
            inst("COMM", 7),
            inst("PUSHREG"),
            inst("PUSHIS", 0x6D9),
            inst("COMM", 7),
        ...
        f020_08_auto_kila_label_offset = (
            len(f020_08_insts) + len(f020_08_insert_insts_autokilacheck) - diff
        )
        f020_08_labels.append(
            assembler.label("AUTO_INSERT_KILA", f020_08_auto_kila_label_offset)
        )
        auto_kila_text = f020_obj.appendMessage(
            "Kilas automatically inserted.", "AUTO_KILA"
        )
        f020_08_insert_insts_autokila_do = [
            inst(
                "END"
            ),  # My math is off by one so instead of making it correct I'm adding what's supposed to be the instruction before to here to make it work. :)
            inst("COMM", 0x60),
            inst("COMM", 1),
            ...
            inst(
                "PUSHIS", 0
            ),  # Mostly copied code. It works but I don't even know what half of this does.
        ...
        f020_08_insts = (
            [f020_08_insts[0]]
            + f020_08_insert_insts_autokilacheck
            + f020_08_insts[1:precut]
            + f020_08_insts[postcut:-1]
            + f020_08_insert_insts_autokila_do
        )
        ...
        f020_proc_waits = [
            ("018_01eve_01", f020_18_01_waits),
            ("018_01eve_02", f020_18_02_waits),
            ("019_01eve_01", f020_19_01_waits),
            ("019_01eve_02", f020_19_02_waits),
            ("019_01eve_03", f020_19_03_waits),
            ("020_01eve_01", f020_20_01_waits),
            ("020_01eve_02", f020_20_02_waits),
            ("020_01eve_03", f020_20_03_waits),
            ("020_01eve_04", f020_20_04_waits),
            ("020_01eve_05", f020_20_05_waits),
            ("020_01eve_06", f020_20_06_waits),
            ("020_01eve_07", f020_20_07_waits),
        ]
        ...
        f020_30_insts, f020_30_labels = f020_obj.getProcInstructionsLabelsByIndex(
            f020_30_proc
        )
        precut = 13
        postcut = 116
        ...
        f020_13_insts, f020_13_labels = f020_obj.getProcInstructionsLabelsByIndex(
            f020_13_proc
        )
        f020_13_insert_insts = [
            inst(
                "PUSHIS", 0x56C
            ),  # I don't know what these flags do, but they are set here.
            inst("COMM", 8),
            inst("PUSHIS", 0x56D),
            inst("COMM", 8),
            inst("PUSHIS", 0x27A),
        ...
        f020_13_insts = (
            f020_13_insts[:precut] + f020_13_insert_insts + f020_13_insts[postcut:]
        )
        if not config_settings.longest_cutscenes:
            f020_obj.changeProcByIndex(f020_13_insts, f020_13_labels, f020_13_proc)

        f020_berith_rwms_index = f020_obj.appendMessage(
            self.get_reward_str("Berith", world), "BERITH_REWARD"
        )
        ...
        f020_kaiwan_rwms_index = f020_obj.appendMessage(
            self.get_reward_str("Kaiwan", world), "KAIWAN_REWARD"
        )
        ...
        self.insert_callback(
            "f020", 0x158, f020_kaiwan_callback_str
        )  # 0x158 is the Kaiwan callback index

        f020_berith_proc = f020_obj.getProcIndexByLabel(
            "024_01eve_01"
        )  # Change berith model to new boss
        f020_berith_insts, f020_berith_labels = (
            f020_obj.getProcInstructionsLabelsByIndex(f020_berith_proc)
        )
        f020_berith_insts[25] = inst("PUSHIS", self.get_checks_boss_id("Berith", world))
        if (
            config_settings.shortest_cutscenes
        ):  # Remove berith cutscene if short setting is on
            precut = 13
            postcut = 178
            diff = postcut - precut
        ...
        f020_obj.changeProcByIndex(
            f020_berith_insts, f020_berith_labels, f020_berith_proc
        )
        f020_eligor_name_id = f020_obj.sections[3].messages[0x16].name_id
        f020_eligor_message = assembler.message(
            "^r"
            + self.get_checks_boss_name("Berith", world)
            + "^p says that's enough...",
            "HEISHI01",
        )
        f020_eligor_message.name_id = f020_eligor_name_id
        f020_obj.changeMessageByIndex(f020_eligor_message, 0x16)

        f020_kaiwan_proc = f020_obj.getProcIndexByLabel(
            "027_start"
        )  # Change kaiwan model to new boss
        f020_kaiwan_insts, f020_kaiwan_labels = (
            f020_obj.getProcInstructionsLabelsByIndex(f020_kaiwan_proc)
        )
        f020_kaiwan_insts[311] = inst(
            "PUSHIS", self.get_checks_boss_id("Kaiwan", world)
        )  # Middle Kaiwan
        f020_kaiwan_insts[323] = inst(
            "PUSHIS", self.get_checks_boss_id("Kaiwan", world, index=1)
        )  # Kaiwan on the left
        f020_kaiwan_insts[335] = inst(
            "PUSHIS", self.get_checks_boss_id("Kaiwan", world, index=2)
        )  # Kaiwan on the right
        if (
            config_settings.shortest_cutscenes
        ):  # Remove berith cutscene if short setting is on
        ...
        f020_obj.changeProcByIndex(
            f020_kaiwan_insts, f020_kaiwan_labels, f020_kaiwan_proc
        )
        f020_obj.changeMessageByIndex(
            assembler.message(
                "> ^r"
                + self.get_checks_boss_name("Kaiwan", world)
                + "^p unlocked the door.^n^x> Will you enter?",
                "F020_KIUNDOOR02",
            ),
            0x6E,
        )
        ...
        f020_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Ose", world)
                + "^p^nbeyond the door.^n^x> Will you enter?",
                "HON_ENT",
            ),
            0x4C,
        )
        ...
        f020_obj.changeNameByLookup(
            "Berith", self.get_checks_boss_name("Berith", world, immersive=True)
        )
        f020_obj.changeNameByLookup(
            "Kaiwan", self.get_checks_boss_name("Kaiwan", world, immersive=True)
        )
        f020_berith_name_id = f020_obj.sections[3].messages[0x22].name_id
        f020_berith_message = assembler.message(
            "Halt...^n^xI am "
            + self.get_checks_boss_name("Berith", world, immersive=True)
            + ", the great duke of hell.^n^xInsolent scoundrel, put down^nthe Kila and begone!",
            "BERI01",
        )
        ...
        e634_80_insts, e634_80_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_80_proc
        )
        ...
        for l in e634_80_labels:
            if l.label_offset > precut:
                ...
                if l.label_offset < 0:
                    l.label_offset = 1
        e634_80_insts = (
            e634_80_insts[:precut]
            + e634_80_insts[postcut:precut2]
            + e634_80_insts[postcut2:]
        )
        ...
        e634_10_5_insts, e634_10_5_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_10_5_proc
        )
        ...
        e634_10_6_insts, e634_10_6_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_10_6_proc
        )
        ...
        e634_10_insts, e634_10_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_10_proc
        )
        ...
        e634_20_insts, e634_20_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_20_proc
        )
        ...
        e634_30_insts, e634_30_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_30_proc
        )
        ...
        e634_main_insts, e634_main_labels = e634_obj.getProcInstructionsLabelsByIndex(
            e634_main_proc
        )
        e634_main_insts[192] = inst("PUSHIS", 0x78)  # Load shorter cutscene
        e634_main_insts[243] = inst(
            "PUSHIS", self.get_checks_boss_id("Ose", world)
        )  # Load demon model in the header
        ...
        for l in e634_main_labels:
            if l.label_offset > precut:
                ...
                if l.label_offset < 1:
                    l.label_offset = 1
        e634_main_insts = (
            e634_main_insts[:248] + e634_main_insert_insts + e634_main_insts[248:]
        )
        e634_main_insts = (
            e634_main_insts[:precut]
            + e634_main_insts[postcut:precut2]
            + e634_main_insts[postcut2:]
        )
        e634_obj.changeProcByIndex(e634_main_insts, e634_main_labels, e634_main_proc)

        e634_obj.changeNameByLookup(
            "Ose", self.get_checks_boss_name("Ose", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e634"], BytesIO(bytes(e634_obj.toBytes()))
        )

        f003_obj = self.get_script_obj_by_name("f003")
        f003_proclen = len(f003_obj.p_lbls().labels)

        f003_ose_callback_message = f003_obj.appendMessage(
            self.get_reward_str("Ose", world), "OSE_REWARD"
        )
        ...
        if not config_settings.longest_cutscenes:
            f004_obj.changeProcByIndex(
                f004_biker_insts, f004_biker_labels, f004_biker_event
            )
        f004_biker_callback_proc_str = "HBIKER_CB"
        f004_biker_callback_msg = f004_obj.appendMessage(
            self.get_reward_str("Hell Biker", world), "HBIKER_REWARD"
        )
        f004_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("Hell Biker", world)
                + "^p.",
                "HBIKER_HINT",
            ),
            4,
        ...
        f004_obj.changeMessageByIndex(
            assembler.message(
                "> East Ikebukuro Station.^n^x> The key is stashed in^n^g"
                + self.get_flag_reward_location_string(0x3F7, world)
                + "^p.",
                "SUBWAY",
            ),
            0x0,
        ...
        f004_wap = bytearray(
            self.dds3.get_file_from_path(custom_vals.WAP_PATH["f004"]).read()
        )
        f004_wap[0x35E] = 0xF7
        f004_wap[0x35F] = 0x03
        ...
        f004_lb.read_lb()
        f004_lb = f004_lb.export_lb(
            {"WAP": BytesIO(f004_wap), "BF": BytesIO(bytes(f004_obj.toBytes()))}
        )
        ...
        e741_10_insts, e741_10_labels = e741_obj.getProcInstructionsLabelsByIndex(
            e741_10_proc
        )
        ...
        e741_main_insts, e741_main_labels = e741_obj.getProcInstructionsLabelsByIndex(
            e741_main_proc
        )
        e741_main_insts[68] = inst(
            "PUSHIS", self.get_checks_boss_id("Hell Biker", world)
        )
        ...
        e741_obj.changeNameByLookup(
            "Hell Biker", self.get_checks_boss_name("Hell Biker", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e741"], BytesIO(bytes(e741_obj.toBytes()))
        )
        ...
        f025_mizuchi_room_insts, f025_mizuchi_room_labels = (
            f025_obj.getProcInstructionsLabelsByIndex(f025_mizuchi_room)
        )
        ...
        if config_settings.shortest_cutscenes:
            ...
            f025_mizuchi_room_insts = (
                f025_mizuchi_room_insts[:precut]
                + f025_mizuchi_room_insert_insts
                + f025_mizuchi_room_insts[postcut:]
            )
        else:
            f025_mizuchi_room_insts[38] = inst(
                "PUSHIS", self.get_checks_boss_id("Mizuchi", world)
            )
            f025_mizuchi_room_insts[84] = inst("PUSHIS", 0x595)
            f025_mizuchi_room_insert_insts = self.get_flag_reward_insts(
                "Mizuchi", world
            )
            f025_mizuchi_room_labels[-1].label_offset += len(
                f025_mizuchi_room_insert_insts
            )
            f025_mizuchi_room_insts = (
                f025_mizuchi_room_insts[:88]
                + f025_mizuchi_room_insert_insts
                + f025_mizuchi_room_insts[88:]
            )
            f025_obj.changeNameByLookup(
                "Mizuchi", self.get_checks_boss_name("Mizuchi", world, immersive=True)
            )
        f025_obj.changeProcByIndex(
            f025_mizuchi_room_insts, f025_mizuchi_room_labels, f025_mizuchi_room
        )
        f025_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Mizuchi", world), "MIZUCHI_REWARD"),
            0x62,
        )
        f025_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Mizuchi", world)
                + "^p^nbeyond the door.^n^x> Will you enter?",
                "F025_DOOR01",
            ),
            0xB4,
        ...
        f025_021_05 = f025_obj.getProcIndexByLabel(
            "021_01eve_05"
        )  # I don't think this gets executed, but I was frustrated when I chose the wrong LB file and this also has the Mizuchi text.
        f025_021_insts = [inst("PROC", f025_021_05), inst("END")]
        ...
        f026_kinki_rwms_index = f026_obj.appendMessage(
            self.get_reward_str("Kin-Ki", world), "KINKI_REWARD"
        )
        ...
        f026_15_room_insts, f026_15_room_labels = (
            f026_obj.getProcInstructionsLabelsByIndex(f026_15_room)
        )
        f026_15_room_insts[7] = inst("PUSHIS", self.get_checks_boss_id("Kin-Ki", world))
        f026_obj.changeProcByIndex(
            f026_15_room_insts, f026_15_room_labels, f026_15_room
        )

        f026_suiki_rwms_index = f026_obj.appendMessage(
            self.get_reward_str("Sui-Ki", world), "SUIKI_REWARD"
        )
        ...
        f026_14_room_insts, f026_14_room_labels = (
            f026_obj.getProcInstructionsLabelsByIndex(f026_14_room)
        )
        f026_14_room_insts[7] = inst("PUSHIS", self.get_checks_boss_id("Sui-Ki", world))
        f026_obj.changeProcByIndex(
            f026_14_room_insts, f026_14_room_labels, f026_14_room
        )

        f026_fuuki_rwms_index = f026_obj.appendMessage(
            self.get_reward_str("Fuu-Ki", world), "FUUKI_REWARD"
        )
        ...
        f026_16_room_insts, f026_16_room_labels = (
            f026_obj.getProcInstructionsLabelsByIndex(f026_16_room)
        )
        f026_16_room_insts[7] = inst("PUSHIS", self.get_checks_boss_id("Fuu-Ki", world))
        f026_obj.changeProcByIndex(
            f026_16_room_insts, f026_16_room_labels, f026_16_room
        )

        f026_ongyoki_rwms_index = f026_obj.appendMessage(
            self.get_reward_str("Ongyo-Ki", world), "ONGYOKI_REWARD"
        )
        ...
        f026_ongyo_proc = f026_obj.getProcIndexByLabel(
            "017_start"
        )  # Change ongyo-ki model to new boss and add hint message
        f026_ongyo_insts, f026_ongyo_labels = f026_obj.getProcInstructionsLabelsByIndex(
            f026_ongyo_proc
        )
        f026_ongyo_insts[17] = inst(
            "PUSHIS", self.get_checks_boss_id("Ongyo-Ki", world)
        )
        if config_settings.shortest_cutscenes:
            precut = 8
            postcut = 140
        ...
        f026_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Ongyo-Ki", world)
                + "^p^nbeyond the door...",
                "F26_FUIN_YOKI",
            ),
            0x2B,
        ...
        f026_obj.changeNameByLookup(
            "Kin-ki", self.get_checks_boss_name("Kin-Ki", world, immersive=True)
        )
        f026_obj.changeNameByLookup(
            "Sui-ki", self.get_checks_boss_name("Sui-Ki", world, immersive=True)
        )
        f026_obj.changeNameByLookup(
            "Fuu-ki", self.get_checks_boss_name("Fuu-Ki", world, immersive=True)
        )
        f026_obj.changeNameByLookup(
            "Ongyo-ki", self.get_checks_boss_name("Ongyo-Ki", world, immersive=True)
        )

        f026_kinki_name_id = f026_obj.sections[3].messages[0x10].name_id
        f026_kinki_message = assembler.message(
            "............^n^x...WHAT DO YOU WANT?^n^xI AM "
            + self.get_checks_boss_name("Kin-Ki", world, immersive=True).upper()
            + ". YOU FILTHY DOG...",
            "F26_KINKI",
        )
        ...
        f026_obj.changeMessageByIndex(
            assembler.message(
                "> "
                + self.get_checks_boss_name("Kin-Ki", world, immersive=True)
                + " is not listening. What will^nyou do?",
                "F26_KINKI_2",
            ),
            0x11,
        )
        f026_obj.changeMessageByIndex(
            assembler.message(
                "> "
                + self.get_checks_boss_name("Kin-Ki", world, immersive=True)
                + "'s breathing is getting heavier.",
                "F26_KINKI_HANA",
            ),
            0x15,
        )
        f026_suiki_name_id = f026_obj.sections[3].messages[0x16].name_id
        f026_suiki_message = assembler.message(
            "...I've never seen you before.^n^xI'm "
            + self.get_checks_boss_name("Sui-Ki", world, immersive=True)
            + ".^n^xI'm a cold demon.",
            "F26_SUIKI",
        )
        ...
        f026_fuuki_message = assembler.message(
            "Yee-haw!^n^xI'm "
            + self.get_checks_boss_name("Fuu-Ki", world, immersive=True)
            + ".^n^xYou must have some time on your^nhands, making it all the way down^nhere.",
            "F26_FUUKI",
        )
        ...
        f026_ongyoki_message = assembler.message(
            "My name is "
            + self.get_checks_boss_name("Ongyo-Ki", world, immersive=True)
            + "...^n^xAre you the one who slew my^npartners and awakened me!?",
            "F26_ONGYO",
        )
        ...
        f027_pr_insts, f027_pr_labels = f027_obj.getProcInstructionsLabelsByIndex(
            f027_pr_proc
        )
        f027_pr_insts[12] = inst(
            "PUSHIS", 0x3F4
        )  # Change rider trigger check from 7b8 to key item
        f027_obj.changeProcByIndex(f027_pr_insts, f027_pr_labels, f027_pr_proc)

        f027_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("Pale Rider", world)
                + "^p.",
                "FIRE_YURE",
            ),
            0x13,
        ...
        if not config_settings.longest_cutscenes:
            ...
            f027_16_insts, f027_16_labels = f027_obj.getProcInstructionsLabelsByIndex(
                f027_16_proc
            )
        ...
        f027_prider_rwms_index = f027_obj.appendMessage(
            self.get_reward_str("Pale Rider", world), "PR_REWARD"
        )
        f027_pr_rwms_insts = (
            [
        ...
        f027_bfrost_rwms_index = f027_obj.appendMessage(
            self.get_reward_str("Black Frost", world), "BFROST_REWARD"
        )
        ...
        f027_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Black Frost", world)
                + "^p^nbeyond the door.^n^x> Will you enter?",
                "F027_DOOR01",
            ),
            0x4E,
        ...
        f027_rider_flame_insts, f027_rider_flame_labels = (
            f027_obj.getProcInstructionsLabelsByIndex(f027_rider_flame_proc)
        )
        f027_rider_flame_insts[6] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f027_obj.changeProcByIndex(
            f027_rider_flame_insts, f027_rider_flame_labels, f027_rider_flame_proc
        )
        f027_rider_fight1_proc = f027_obj.getProcIndexByLabel("001_01eve_01")
        f027_rider_fight1_insts, f027_rider_fight1_labels = (
            f027_obj.getProcInstructionsLabelsByIndex(f027_rider_fight1_proc)
        )
        f027_rider_fight1_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f027_obj.changeProcByIndex(
            f027_rider_fight1_insts, f027_rider_fight1_labels, f027_rider_fight1_proc
        )
        f027_rider_fight2_proc = f027_obj.getProcIndexByLabel("001_01eve_02")
        f027_rider_fight2_insts, f027_rider_fight2_labels = (
            f027_obj.getProcInstructionsLabelsByIndex(f027_rider_fight2_proc)
        )
        f027_rider_fight2_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f027_obj.changeProcByIndex(
            f027_rider_fight2_insts, f027_rider_fight2_labels, f027_rider_fight2_proc
        )
        f027_rider_fight3_proc = f027_obj.getProcIndexByLabel(
            "001_01eve_03"
        )  # There are 3 Asakusa rider battles because why?
        f027_rider_fight3_insts, f027_rider_fight3_labels = (
            f027_obj.getProcInstructionsLabelsByIndex(f027_rider_fight3_proc)
        )
        f027_rider_fight3_insts[1] = inst(
            "PUSHIS", 0x113
        )  # Switch flag to pale rider fought so it's never relevant
        f027_obj.changeProcByIndex(
            f027_rider_fight3_insts, f027_rider_fight3_labels, f027_rider_fight3_proc
        )
        ...
        e746_10_insts, e746_10_labels = e746_obj.getProcInstructionsLabelsByIndex(
            e746_10_proc
        )
        ...
        e746_10_insts[84] = inst(
            "PUSHSTR", 138
        )  # Fix camera to show the boss at all times
        e746_10_insts = (
            e746_10_insts[:139] + e746_10_insts[150:161] + e746_10_insts[176:]
        )  # Fix camera
        ...
        e746_main_insts, e746_main_labels = e746_obj.getProcInstructionsLabelsByIndex(
            e746_main_proc
        )
        e746_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("Pale Rider", world)
        )
        ...
        e746_obj.changeNameByLookup(
            "Pale Rider", self.get_checks_boss_name("Pale Rider", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e746"], BytesIO(bytes(e746_obj.toBytes()))
        )
        ...
        e644_labels = [assembler.label("BFROST_FOUGHT", 13)]
        e644_obj.changeProcByIndex(e644_insts, e644_labels, 0)
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e644"], BytesIO(bytes(e644_obj.toBytes()))
        )
        ...
        f039_obj.changeNameByLookup(
            "Bishamon", self.get_checks_boss_name("Bishamon 1", world, immersive=True)
        )
        f039_obj.changeMessageByIndex(
            assembler.message("Well done.", "SHORTER_B_TEXT"), 0x11
        )
        f039_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Bishamon 1", world), "BISHA_REWARD"),
            0x13,
        )
        f039_obj.changeMessageByIndex(
            assembler.message(
                "> Do you accept "
                + self.get_checks_boss_name("Bishamon 1", world, immersive=True)
                + "'s^nchallenge?",
                "f039_BOSS01_03",
            ),
            0xD,
        ...
        f039_rwms_insts, f039_rwms_labels = f039_obj.getProcInstructionsLabelsByIndex(
            f039_rwms_proc
        )
        ...
        f039_rwms_insts[23] = inst(
            "PUSHIS", self.get_checks_boss_id("Bishamon 1", world)
        )
        f039_rwms_insts = (
            f039_rwms_insts[0:2]
            + self.get_flag_reward_insts("Bishamon 1", world)
            + f039_rwms_insts[2:-1]
            + [inst("END")]
        )
        f039_obj.changeProcByIndex(
            f039_rwms_insts, [], f039_rwms_proc
        )  # No labels in the proc
        f039_02_proc = f039_obj.getProcIndexByLabel("002_start")
        f039_02_insts, f039_02_labels = f039_obj.getProcInstructionsLabelsByIndex(
            f039_02_proc
        )
        f039_02_insts[41] = inst("PUSHIS", self.get_checks_boss_id("Bishamon 1", world))
        f039_02_insts[166] = inst(
            "PUSHIS", self.get_checks_boss_id("Bishamon 1", world)
        )
        ...
        f035_09_insts, f035_09_labels = f035_obj.getProcInstructionsLabelsByIndex(
            f035_09_index
        )
        if (
            not config_settings.longest_cutscenes
        ):  # Immediately start fight if cutscenes are off
            ...
            f035_09_insts = (
                f035_09_insts[:precut]
                + f035_futomimi_insert_insts
                + f035_09_insts[postcut:]
            )
        if (
            config_settings.open_yurakucho
        ):  # Use flag that we know is off until after both fights
        ...
        f035_obj.changeMessageByIndex(
            assembler.message(
                "> The mirror's reflection shows^n^r"
                + self.get_checks_boss_name("Futomimi", world)
                + "^p and ^r"
                + self.get_checks_boss_name("Archangels", world)
                + "^p.^n^x> Will you step into the light?",
                "HIKARI",
            ),
            0xA,
        )

        f035_futomimi_rwms_index = f035_obj.appendMessage(
            self.get_reward_str("Futomimi", world), "FUTO_RWMS"
        )
        f035_angel_rwms_index = f035_obj.appendMessage(
            self.get_reward_str("Archangels", world), "ANGE_RWMS"
        )
        f035_futomimi_callback_insts = (
            [
                ...
                inst(
                    "PUSHIS", 0x81
                ),  # Use Agree with Chiaki flag because it is not needed elsewhere
                inst("COMM", 0x7),
                inst("PUSHREG"),
                inst("EQ"),
                inst("IF", 0),
                ...
                inst(
                    "PUSHIS", 0x23
                ),  # Set a callback to self but check the Chiaki agree flag to prevent an infinite loop
                inst("PUSHIS", 1),
                inst("COMM", 0x97),
                inst(
                    "PUSHIS", 0x2A1
                ),  # This is awkward, but it is the best I can do for now. Archangels are boss rush after futomimi
                inst("COMM", 0x67),
                inst("END"),
                inst(
                    "COMM", 0x60
                ),  # If this is after the angel fight, show the reward message and exit
                inst("COMM", 1),
                inst("PUSHIS", f035_angel_rwms_index),
                inst("COMM", 0),
                inst("COMM", 2),
        ...
        f035_futomimi_callback_labels = [
            assembler.label(
                "ANGEL_FOUGHT", 22
            )  # + len(self.get_flag_reward_insts("Futomimi",world)))
        ]
        f035_futomimi_callback_str = "FUTO_CB"
        f035_obj.appendProc(
            f035_futomimi_callback_insts,
            f035_futomimi_callback_labels,
            f035_futomimi_callback_str,
        )
        ...
        e672_main_insts, e672_main_labels = e672_obj.getProcInstructionsLabelsByIndex(
            e672_main_proc
        )
        e672_main_insts[24] = inst(
            "PUSHIS", self.get_checks_boss_id("Futomimi", world)
        )  # Model swaps
        e672_main_insts[25] = inst("PUSHIS", 0x6)
        e672_main_insts[26] = inst("COMM", 0x15)
        e672_main_insts[34] = inst(
            "PUSHIS", self.get_checks_boss_id("Archangels", world)
        )
        e672_main_insts[35] = inst("PUSHIS", 0x6)
        e672_main_insts[36] = inst("COMM", 0x15)
        e672_main_insts[39] = inst(
            "PUSHIS", self.get_checks_boss_id("Archangels", world, index=1)
        )
        e672_main_insts[40] = inst("PUSHIS", 0x6)
        e672_main_insts[41] = inst("COMM", 0x15)
        e672_main_insts[44] = inst(
            "PUSHIS", self.get_checks_boss_id("Archangels", world, index=2)
        )
        e672_main_insts[45] = inst("PUSHIS", 0x6)
        e672_main_insts[46] = inst("COMM", 0x15)
        e672_main_insts[208] = inst(
            "PUSHIS", 0x2
        )  # Attempt to fix music playing in fight
        e672_main_insts[211] = inst("PUSHIS", 0x56)  # Flag to open yurakucho
        e672_main_insts[319] = inst("PUSHIS", 0x0)
        e672_main_insts[334] = inst(
            "PUSHIS", 0x2A0
        )  # Play event again instead of baal transformation
        ...
        e672_main_insts = (
            [e672_main_insts[0]]
            + e672_main_insert_insts_start
            + e672_main_insts[1:insert_index]
            + e672_main_insert_insts
            + e672_main_insts[insert_index:insert_index_2]
            + e672_main_insert_insts_2
            + e672_main_insts[insert_index_2:precut]
            + e672_main_insts[postcut:insert_index_3]
            + e672_main_insert_insts_3
            + e672_main_insts[insert_index_3:insert_index_4]
            + e672_main_insert_insts_4
            + e672_main_insts[insert_index_4:]
        )
        e672_obj.changeProcByIndex(e672_main_insts, e672_main_labels, e672_main_proc)

        e672_100_proc = e672_obj.getProcIndexByLabel(
            "e672_100"
        )  # Fix camera and futomimi animations
        e672_100_insts, e672_100_labels = e672_obj.getProcInstructionsLabelsByIndex(
            e672_100_proc
        )
        ...
        e672_100_insts = (
            e672_100_insts[:75] + e672_100_insts[89:]
        )  # Inteferes with animations
        e672_obj.changeProcByIndex(e672_100_insts, e672_100_labels, e672_100_proc)

        e672_futomimi_rwms_index = e672_obj.appendMessage(
            self.get_reward_str("Futomimi", world), "FUTO_RWMS"
        )  # Chiaki "gives" reward, but real logic is in the callback

        e672_110_proc = e672_obj.getProcIndexByLabel(
            "e672_110"
        )  # Give reward message instead of Chiaki's disappointment
        e672_110_insts, e672_110_labels = e672_obj.getProcInstructionsLabelsByIndex(
            e672_110_proc
        )
        ...
        e672_110_insts = (
            [e672_110_insts[0]] + e672_110_insert_insts + e672_110_insts[1:]
        )
        e672_obj.changeProcByIndex(e672_110_insts, e672_110_labels, e672_110_proc)

        e672_obj.changeNameByLookup(
            "Futomimi", self.get_checks_boss_name("Futomimi", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e672"], BytesIO(bytes(e672_obj.toBytes()))
        )
        ...
        f031_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Sisters", world), "SIS_REWARD"), 0x2D
        )

        f031_rwms_proc = f031_obj.getProcIndexByLabel("000_dh_plus")
        f031_rwms_insts, f031_rwms_labels = f031_obj.getProcInstructionsLabelsByIndex(
            f031_rwms_proc
        )
        f031_rwms_insts = (
            f031_rwms_insts[:-1]
            + self.get_flag_reward_insts("Sisters", world)
            + [inst("END")]
        )
        f031_obj.changeProcByIndex(f031_rwms_insts, [], f031_rwms_proc)

        f031_sisters_proc = f031_obj.getProcIndexByLabel(
            "012_start"
        )  # Change sisters models to new boss and add hint message
        f031_sisters_insts, f031_sisters_labels = (
            f031_obj.getProcInstructionsLabelsByIndex(f031_sisters_proc)
        )
        f031_sisters_insts[100] = inst(
            "PUSHIS", self.get_checks_boss_id("Sisters", world)
        )
        f031_sisters_insts[112] = inst(
            "PUSHIS", self.get_checks_boss_id("Sisters", world, index=2)
        )
        f031_sisters_insts[124] = inst(
            "PUSHIS", self.get_checks_boss_id("Sisters", world, index=1)
        )
        f031_obj.changeProcByIndex(
            f031_sisters_insts, f031_sisters_labels, f031_sisters_proc
        )
        f031_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Sisters", world)
                + "^p^non the floor above.^n^x> Will you go up?",
                "BOSS_ROOM_IN",
            ),
            0x32,
        )
        f031_obj.changeNameByLookup(
            "Clotho",
            self.get_checks_boss_name("Sisters", world, immersive=True, index=2),
        )
        f031_obj.changeNameByLookup(
            "Lachesis", self.get_checks_boss_name("Sisters", world, immersive=True)
        )
        f031_obj.changeNameByLookup(
            "Atropos",
            self.get_checks_boss_name("Sisters", world, immersive=True, index=1),
        )

        if config_settings.shortest_cutscenes:  # Remove boss cutscene
            f031_boss_proc = f031_obj.getProcIndexByLabel(
                "012_01eve_04"
            )  # Change sisters models to new boss and add hint message
            f031_boss_insts, f031_boss_labels = (
                f031_obj.getProcInstructionsLabelsByIndex(f031_boss_proc)
            )
            precut = 81
            postcut = 170
            diff = postcut - precut
            ...
            f031_obj.changeProcByIndex(
                f031_boss_insts, f031_boss_labels, f031_boss_proc
            )

        f031_obj.changeMessageByIndex(
            assembler.message(
                "> Will you go to the Tower of^n^r"
                + self.get_checks_boss_name("Kagutsuchi", world)
                + "^p",
                "f031_GO_KAGUTUTI",
            ),
            0x30,
        ...
        e651_obj = self.get_script_obj_by_name(
            "e651"
        )  # Remove saving yuko in obelisk cutscene
        e651_insts = [
            inst("PROC", 0),
            inst("PUSHIS", 0x10),
            inst("PUSHIS", 1),
        ...
        e651_obj.changeMessageByIndex(
            assembler.message("You have no business here.", "MSG_030_1"), 0x0
        )
        e651_obj.changeProcByIndex(e651_insts, [], 0)
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e651"], BytesIO(bytes(e651_obj.toBytes()))
        )
        ...
        f028_011_insts, f028_011_labels = f028_obj.getProcInstructionsLabelsByIndex(
            f028_011_index
        )
        precut = 7
        postcut = 163
        ...
        if config_settings.shortest_cutscenes:
            ...
            f028_011_insts[15] = inst(
                "PUSHIS", self.get_checks_boss_id("Specter 2", world)
            )
            f028_obj.changeNameByLookup(
                "Specter", self.get_checks_boss_name("Specter 2", world, immersive=True)
            )
        f028_obj.changeProcByIndex(f028_011_insts, f028_011_labels, f028_011_index)
        f028_specter2_rwms_index = f028_obj.appendMessage(
            self.get_reward_str("Specter 2", world), "SPEC2_RWMS"
        )
        ...
        f028_obj.appendProc(
            f028_specter2_callback_insts, [], f028_specter2_callback_str
        )
        ...
        e652_terminal_label_index = (
            0  # relative index for TERMINAL label. Absolute is 13
        )
        e652_kept_insts = e652_insts[
            :52
        ]  # 0-31 is terminal code. 32-51 has camera to hijiri code.
        e652_network2_msg = e652_obj.appendMessage(
            "I think a "
            + self.get_checks_boss_name("Specter 2", world)
            + " is in Amala Network 2. Go?",
            "NETWORK2_MSG",
        )
        e652_network3_msg = e652_obj.appendMessage(
            self.get_checks_boss_name("Specter 3", world)
            + " is pulling me in! ...Save Hijiri?",
            "NETWORK3_MSG",
        )
        e652_locked_msg = e652_obj.appendMessage(
            "Come back after you've completed Yoyogi Park and I will take you to Amala Network 3.",
            "LOCKED_MSG",
        )
        ...
        e652_insert_insts = [
            ...
            inst(
                "END"
            ),  # end label location. Also here to not trip off a warning in the assembler.
        ]
        ...
        e652_obj.changeProcByIndex(
            e652_kept_insts + e652_insert_insts, e652_labels, e652_proc
        )
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e652"], BytesIO(bytes(e652_obj.toBytes()))
        )
        ...
        e660_terminal_label_index = (
            0  # relative index for TERMINAL label. Absolute is 13
        )
        e660_kept_insts = e660_insts[
            :52
        ]  # 0-31 is terminal code. 32-51 has camera to hijiri code.
        e660_network2_msg = e660_obj.appendMessage(
            "I think a "
            + self.get_checks_boss_name("Specter 2", world)
            + " is in Amala Network 2. Go?",
            "NETWORK2_MSG",
        )
        e660_network3_msg = e660_obj.appendMessage(
            self.get_checks_boss_name("Specter 3", world)
            + " is pulling me in! ...Save Hijiri?",
            "NETWORK3_MSG",
        )
        e660_locked_msg = e660_obj.appendMessage(
            "Come back after you've completed Yoyogi Park and I will take you to Amala Network 3.",
            "LOCKED_MSG",
        )
        e660_gl_msg = e660_obj.appendMessage("Good luck!", "GL_MSG")
        e660_yesno_msg = e660_obj.appendMessage(
            "Yes^t^.No^t", "YES_NO_SEL", is_decision=True
        )
        e660_insert_insts = [
            inst("PUSHIS", 0),
            inst("PUSHIS", 0x5F9),  # Check if gone in Network 2
        ...
        e660_obj.changeProcByIndex(
            e660_kept_insts + e660_insert_insts, e660_labels, e660_proc
        )
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e660"], BytesIO(bytes(e660_obj.toBytes()))
        )
        ...
        e658_labels = [assembler.label("GARY_FOUGHT", 13)]
        if not config_settings.longest_cutscenes:
            e658_obj.changeProcByIndex(e658_insts, e658_labels, 0)
        else:
            e658_main_proc = e658_obj.getProcIndexByLabel("e658_main")
            e658_main_insts, e658_main_labels = (
                e658_obj.getProcInstructionsLabelsByIndex(e658_main_proc)
            )
            e658_main_insts[27] = inst(
                "PUSHIS", self.get_checks_boss_id("Girimehkala", world)
            )
            ...
            for l in e658_main_labels:
                if l.label_offset > precut:
                    l.label_offset -= diff
            e658_main_insts = (
                e658_main_insts[:precut]
                + e658_main_insts[postcut:precut2]
                + e658_main_insts[postcut2:]
            )
            e658_obj.changeNameByLookup(
                "Sakahagi",
                self.get_checks_boss_name("Girimehkala", world, immersive=True),
            )
            e658_obj.changeProcByIndex(
                e658_main_insts, e658_main_labels, e658_main_proc
            )

            e658_10_proc = e658_obj.getProcIndexByLabel("e658_010")
            e658_10_insts, e658_10_labels = e658_obj.getProcInstructionsLabelsByIndex(
                e658_10_proc
            )
            ...
            e658_10_insts = (
                e658_10_insts[0:20] + e658_10_insert_insts + e658_10_insts[20:]
            )
            e658_obj.changeProcByIndex(e658_10_insts, e658_10_labels, e658_10_proc)

            e658_20_proc = e658_obj.getProcIndexByLabel("e658_020")
            e658_20_insts, e658_20_labels = e658_obj.getProcInstructionsLabelsByIndex(
                e658_20_proc
            )
            ...
            e658_30_insts, e658_30_labels = e658_obj.getProcInstructionsLabelsByIndex(
                e658_30_proc
            )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e658"], BytesIO(bytes(e658_obj.toBytes()))
        )
        ...
        f016_gary_reward_msg = f016_obj.appendMessage(
            self.get_reward_str("Girimehkala", world), "GARY_MSG"
        )
        ...
        # Gary hint
        f016_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Girimehkala", world)
                + "^p^nbeyond the door.^n^x> Will you enter?",
                "F016_DOOR01",
            ),
            0x8A,
        ...
        f016_mh_insts, f016_mh_labels = f016_obj.getProcInstructionsLabelsByIndex(
            f016_mh_proc
        )
        f016_mh_insts[1] = inst(
            "PUSHIS", 0x3F5
        )  # Change Harlot trigger from a story trigger to a key item
        ...
        if not config_settings.longest_cutscenes:
            ...
            f016_19_insts, f016_19_labels = f016_obj.getProcInstructionsLabelsByIndex(
                f016_19_proc
            )
        ...
        f016_harlot_rwms_index = f016_obj.appendMessage(
            self.get_reward_str("The Harlot", world), "HARLOT_REWARD"
        )
        ...
        f016_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("The Harlot", world)
                + "^p.",
                "FIRE_YURE",
            ),
            0x4A,
        ...
        f016_demon_message = assembler.message(
            "I want a golden goblet!^n^xI hear you can find one^nat ^g"
            + self.get_flag_reward_location_string(0x3F5, world)
            + "^p.",
            "F016_DEVIL03",
        )
        ...
        f016_pixie_insts, f016_pixie_labels = f016_obj.getProcInstructionsLabelsByIndex(
            f016_pixie_proc
        )
        ...
        f016_pixie_insts = (
            f016_pixie_insts[:f016_pixie_cutoff] + f016_pixie_insert_insts
        )
        ...
        e747_10_insts, e747_10_labels = e747_obj.getProcInstructionsLabelsByIndex(
            e747_10_proc
        )
        ...
        for l in e747_10_labels:
            l.label_offset -= diff
        e747_10_insts = (
            e747_10_insts[:precut]
            + e747_10_insts[postcut:precut2]
            + e747_10_insts[postcut2:precut3]
            + e747_10_insts[postcut3:precut4]
            + e747_10_insts[postcut4:]
        )
        ...
        e747_main_insts, e747_main_labels = e747_obj.getProcInstructionsLabelsByIndex(
            e747_main_proc
        )
        e747_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("The Harlot", world)
        )
        e747_main_insts[55] = inst("PUSHIS", 0x6)
        e747_main_insts[56] = inst("COMM", 0x15)
        e747_main_insts[59] = inst(
            "PUSHIS", self.get_checks_boss_id("The Harlot", world, index=2)
        )
        ...
        e747_mh_message = assembler.message(
            "...and that's me--the "
            + self.get_checks_boss_name("The Harlot", world, immersive=True)
            + ",^nsubjugator of the most fearsome^nbeast.",
            "MSG_003",
        )
        e747_mh_message.name_id = e747_mh_name_id
        e747_obj.changeMessageByIndex(e747_mh_message, 0x2)
        e747_obj.changeNameByLookup(
            "Mother Harlot",
            self.get_checks_boss_name("The Harlot", world, immersive=True),
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e747"], BytesIO(bytes(e747_obj.toBytes()))
        )
        ...
        f030_specter3_rwms_index = f030_obj.appendMessage(
            self.get_reward_str("Specter 3", world), "SPECTER3_REWARD"
        )
        ...
        f030_specter_proc = f030_obj.getProcIndexByLabel(
            "002_01eve_01"
        )  # Change specter model to new boss
        f030_specter_insts, f030_specter_labels = (
            f030_obj.getProcInstructionsLabelsByIndex(f030_specter_proc)
        )
        if config_settings.shortest_cutscenes:
            precut = 7
            postcut = 193
            ...
            for l in f030_specter_labels:
                if l.label_offset > precut:
                    l.label_offset -= diff
            f030_specter_insts = (
                f030_specter_insts[:precut] + f030_specter_insts[postcut:]
            )
        else:
            f030_specter_insts[23] = inst(
                "PUSHIS", self.get_checks_boss_id("Specter 3", world)
            )
            f030_specter_insts[129] = inst(
                "PUSHIS", 4
            )  # Change crashing animation to "spell"
        f030_obj.changeProcByIndex(
            f030_specter_insts, f030_specter_labels, f030_specter_proc
        )
        f030_obj.changeNameByLookup(
            "Specter", self.get_checks_boss_name("Specter 3", world, immersive=True)
        )
        ...
        f034_02_insts, f034_02_labels = f034_obj.getProcInstructionsLabelsByIndex(
            f034_02_proc
        )
        precut = 151
        postcut = 299
        diff = postcut - precut
        f034_02_insert_insts = [inst("PUSHIS", 0x4A0), inst("COMM", 8)]
        f034_02_insts = (
            f034_02_insts[:precut] + f034_02_insert_insts + f034_02_insts[postcut:]
        )
        ...
        f034_obj.changeProcByIndex(
            [inst("PROC", f034_02_1_proc), inst("END")], [], f034_02_1_proc
        )
        f034_obj.changeProcByIndex(
            [inst("PROC", f034_02_2_proc), inst("END")], [], f034_02_2_proc
        )
        f034_obj.changeProcByIndex(
            [inst("PROC", f034_02_3_proc), inst("END")], [], f034_02_3_proc
        )
        ...
        wap_empty_entry = (
            bytes([0] * 0x38)
            + bytes(assembler.ctobb("01pos_01", 8))
            + bytes([0] * 6)
            + bytes(assembler.ctobb("01cam_01", 8))
            + bytes([0, 0, 0, 0, 1, 1])
            + bytes([0] * 0x10)
        )
        ...
        f034_wap_file = (
            f034_wap_file[: 0xA0 + 0x64]
            + wap_empty_entry
            + f034_wap_file[0xA0 + (0x64 * 2) : 0xA0 + (0x64 * 3)]
            + wap_empty_entry
            + f034_wap_file[0xA0 + (0x64 * 4) : 0xA0 + (0x64 * 5)]
            + wap_empty_entry
            + f034_wap_file[0xA0 + (0x64 * 6) :]
        )
        ...
        self.dds3.add_new_file(
            f034_wap_file_path, BytesIO(f034_wap_file)
        )  # Write in normal WAP spot, but this isn't what we 'really' need.
        ...
        f034_temple_bosses_done_msg = f034_obj.appendMessage(
            "All temple bosses defeated.^nThe central temple is now open.",
            "CENTER_TEMPLE_MSG",
        )
        ...
        f034_albion_rwms = f034_obj.appendMessage(
            self.get_reward_str("Albion", world), "ALBION_RWMS"
        )  # Could change a message, but this is just easier.
        f034_25_2_insts = (
            [
        ...
        f034_25_2_labels = [assembler.label("ALBION_TO_CENTER", 19)]
        f034_obj.changeProcByIndex(f034_25_2_insts, f034_25_2_labels, f034_25_2_proc)

        f034_albion_hint_msg = f034_obj.appendMessage(
            "You sense the presence of^n^r"
            + self.get_checks_boss_name("Albion", world)
            + "^p.",
            "ALBION_HINT",
        )
        f034_25_start_proc = f034_obj.getProcIndexByLabel("025_start")
        f034_25_start_insts, f034_25_start_labels = (
            f034_obj.getProcInstructionsLabelsByIndex(f034_25_start_proc)
        )
        f034_25_start_insts[100] = inst("PUSHIS", f034_albion_hint_msg)
        f034_obj.changeProcByIndex(
            f034_25_start_insts, f034_25_start_labels, f034_25_start_proc
        )

        # Skadi callback
        # Set 6ad, check 6ac and 6ae. If both are set then set 6af
        f034_18_2_proc = f034_obj.getProcIndexByLabel("018_01eve_02")
        f034_skadi_rwms = f034_obj.appendMessage(
            self.get_reward_str("Skadi", world), "SKADI_RWMS"
        )
        f034_18_2_insts = (
            [
        ...
        f034_18_2_labels = [assembler.label("SKADI_TO_CENTER", 19)]
        f034_obj.changeProcByIndex(f034_18_2_insts, f034_18_2_labels, f034_18_2_proc)

        f034_skadi_hint_msg = f034_obj.appendMessage(
            "You sense the presence of^n^r"
            + self.get_checks_boss_name("Skadi", world)
            + "^p.",
            "SKADI_HINT",
        )
        f034_18_start_proc = f034_obj.getProcIndexByLabel("018_start")
        f034_18_start_insts, f034_18_start_labels = (
            f034_obj.getProcInstructionsLabelsByIndex(f034_18_start_proc)
        )
        f034_18_start_insts[116] = inst("PUSHIS", f034_skadi_hint_msg)
        f034_obj.changeProcByIndex(
            f034_18_start_insts, f034_18_start_labels, f034_18_start_proc
        )

        # Aciel callback
        f034_10_2_proc = f034_obj.getProcIndexByLabel("010_01eve_02")
        # Set 6ac, check 6ae and 6ad. If both are set then set 6af
        f034_aciel_rwms = f034_obj.appendMessage(
            self.get_reward_str("Aciel", world), "ACIEL_RWMS"
        )
        f034_10_2_insts = (
            [
        ...
        f034_10_2_labels = [assembler.label("ACIEL_TO_CENTER", 19)]
        f034_obj.changeProcByIndex(f034_10_2_insts, f034_10_2_labels, f034_10_2_proc)

        f034_aciel_hint_msg = f034_obj.appendMessage(
            "You sense the presence of^n^r"
            + self.get_checks_boss_name("Aciel", world)
            + "^p.",
            "ACIEL_HINT",
        )
        f034_10_start_proc = f034_obj.getProcIndexByLabel("010_start")
        f034_10_start_insts, f034_10_start_labels = (
            f034_obj.getProcInstructionsLabelsByIndex(f034_10_start_proc)
        )
        f034_10_start_insts[96] = inst("PUSHIS", f034_aciel_hint_msg)
        f034_obj.changeProcByIndex(
            f034_10_start_insts, f034_10_start_labels, f034_10_start_proc
        )

        f034_aciel_proc = f034_obj.getProcIndexByLabel(
            "010_01eve_01"
        )  # Change aciel model to new boss
        f034_aciel_insts, f034_aciel_labels = f034_obj.getProcInstructionsLabelsByIndex(
            f034_aciel_proc
        )
        if config_settings.shortest_cutscenes:
            precut = 7
            ...
            f034_aciel_insts[55] = inst(
                "PUSHIS", self.get_checks_boss_id("Aciel", world)
            )
        f034_obj.changeProcByIndex(f034_aciel_insts, f034_aciel_labels, f034_aciel_proc)
        f034_obj.changeNameByLookup(
            "Aciel", self.get_checks_boss_name("Aciel", world, immersive=True)
        )

        f034_skadi_proc = f034_obj.getProcIndexByLabel(
            "018_01eve_01"
        )  # Change skadi model to new boss
        f034_skadi_insts, f034_skadi_labels = f034_obj.getProcInstructionsLabelsByIndex(
            f034_skadi_proc
        )
        if config_settings.shortest_cutscenes:
            precut = 7
            ...
            f034_skadi_insts[55] = inst(
                "PUSHIS", self.get_checks_boss_id("Skadi", world)
            )
        f034_obj.changeProcByIndex(f034_skadi_insts, f034_skadi_labels, f034_skadi_proc)
        f034_obj.changeNameByLookup(
            "Skadi", self.get_checks_boss_name("Skadi", world, immersive=True)
        )

        f034_albion_proc = f034_obj.getProcIndexByLabel(
            "025_01eve_01"
        )  # Change albion model to new boss
        f034_albion_insts, f034_albion_labels = (
            f034_obj.getProcInstructionsLabelsByIndex(f034_albion_proc)
        )
        if config_settings.shortest_cutscenes:
            precut = 7
            postcut = 126
            ...
            f034_albion_insts[57] = inst(
                "PUSHIS", self.get_checks_boss_id("Albion", world)
            )
        f034_obj.changeProcByIndex(
            f034_albion_insts, f034_albion_labels, f034_albion_proc
        )
        f034_obj.changeNameByLookup(
            "Albion", self.get_checks_boss_name("Albion", world, immersive=True)
        )
        ...
        f034_03_01_insts, f034_03_01_labels = f034_obj.getProcInstructionsLabelsByIndex(
            f034_03_01_proc
        )
        f034_03_01_insts[61] = inst(
            "PUSHIS", 0x3DA
        )  # Change flag check to key item (Himorogi)
        f034_obj.changeProcByIndex(f034_03_01_insts, f034_03_01_labels, f034_03_01_proc)

        # Pyramidion hint
        f034_obj.changeMessageByIndex(
            assembler.message(
                "> What you seek is at^n^g"
                + self.get_flag_reward_location_string(0x3DA, world)
                + "^p.",
                "CENTER_DONE",
            ),
            0x26,
        ...
        #    MSG 1 - 0x12
        #    MSG 2 - 0x13
        f034_inf_patched = BytesIO(
            bytes(open(path.join(PATCHES_PATH, "Doors_F034.INF"), "rb").read())
        )
        self.dds3.add_new_file("/fld/f/f034/F034.INF", f034_inf_patched)
        f034_obj.changeMessageByIndex(
            assembler.message(
                "This door is locked by the^n^bblack temple key^p,^nwhich is found in ^g"
                + self.get_flag_reward_location_string(0x3F1, world)
                + "^p.",
                "BLACK_LOCK",
            ),
            0x10,
        ...
        f034_obj.changeMessageByIndex(
            assembler.message(
                "This door is locked by the^n^ywhite temple key^p,^nwhich is found in ^g"
                + self.get_flag_reward_location_string(0x3F2, world)
                + "^p.",
                "WHITE_LOCK",
            ),
            0x11,
        ...
        f034_obj.changeMessageByIndex(
            assembler.message(
                "This door is locked by the^n^rred temple key^p,^nwhich is found in ^g"
                + self.get_flag_reward_location_string(0x3F3, world)
                + "^p.",
                "RED_LOCK",
            ),
            0x12,
        ...
        f034_obj.changeMessageByIndex(
            assembler.message("You have opened the locked door.", "DOOR_OPEN"), 0x13
        )
        ...
        f034a_lb = f034a_lb.export_lb(
            {
                "WAP": BytesIO(f034_wap_file),
                "BF": BytesIO(bytes(f034_obj.toBytes())),
                "ATMP": f034_amb_a,
                "INF": f034_inf_patched,
            }
        )
        f034b_lb = f034b_lb.export_lb(
            {
                "WAP": BytesIO(f034_wap_file),
                "BF": BytesIO(bytes(f034_obj.toBytes())),
                "ATMP": f034_amb_b,
        ...
        e703_msg = e703_obj.appendMessage(
            "The Tower of ^r"
            + self.get_checks_boss_name("Kagutsuchi", world)
            + "^p has been lowered onto the Obelisk.^nThe Labyrinth of Amala is now closed.",
            "TOK_LOWERED",
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e703"], BytesIO(bytes(e703_obj.toBytes()))
        )
        ...
        if not config_settings.longest_cutscenes:
            f021_obj.changeProcByIndex(
                f021_toot_insts, f021_toot_labels, f021_toot_proc
            )

        f021_toot_rwms = f021_obj.appendMessage(
            self.get_reward_str("Trumpeter", world), "TOOT_RWMS"
        )
        ...
        f021_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("Trumpeter", world)
                + "^p.",
                "FIRE_YURE",
            ),
            0x38,
        ...
        e748_10_insts, e748_10_labels = e748_obj.getProcInstructionsLabelsByIndex(
            e748_10_proc
        )
        ...
        e748_10_insts = (
            e748_10_insts[:316] + e748_10_insts[331:347] + e748_10_insts[358:]
        )  # Fix camera
        ...
        e748_main_insts, e748_main_labels = e748_obj.getProcInstructionsLabelsByIndex(
            e748_main_proc
        )
        e748_main_insts[54] = inst(
            "PUSHIS", self.get_checks_boss_id("Trumpeter", world)
        )
        ...
        e748_doot_message = assembler.message(
            "Little lamb that struggled through^ndarkness and clung to life,^nyou have done well until now.^x...I watched as you prevailed^nover many adversaries.^n^xI am the "
            + self.get_checks_boss_name("Trumpeter", world, immersive=True)
            + "...^nIt is I who will announce^nthe end of time.",
            "MSG_003",
        )
        e748_doot_message.name_id = e748_doot_name_id
        e748_obj.changeMessageByIndex(e748_doot_message, 0x0)
        e748_obj.changeNameByLookup(
            "Trumpeter", self.get_checks_boss_name("Trumpeter", world, immersive=True)
        )

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e748"], BytesIO(bytes(e748_obj.toBytes()))
        )
        ...
        f033_surt_rwms = f033_obj.appendMessage(
            self.get_reward_str("Surt", world), "SURT_RWMS"
        )
        ...
        # Diet building hints
        f033_obj.changeMessageByIndex(
            assembler.message(
                "> There is a statue of ^r"
                + self.get_checks_boss_name("Surt", world)
                + "^p.",
                "F033_DOZO01_02",
            ),
            0x57,
        ...
        f033_surt_message = assembler.message(
            "I am the guardian of this building.^n^xThou shan't reach ^r"
            + self.get_checks_boss_name("Mada", world)
            + "^p.",
            "ITAGAKI_DOUZOU_1",
        )
        ...
        f033_mada_message = assembler.message(
            "...But, I cannot let you get to^n^r"
            + self.get_checks_boss_name("Mot", world)
            + "^p.^n^xThe Magatsuhi in this building is^nalready ours.^n^xAll we have to do now is summon^nour god, and we will receive^nthe Reason of Shijima!^nI will not allow you to meddle with^nour commander.^n^xI, "
            + self.get_checks_boss_name("Mada", world, immersive=True)
            + ", will personally hand you^na ticket to hell!",
            "OOKUMA_DOUZOU",
        )
        ...
        f033_mot_message = assembler.message(
            "Curse thee...^n^xCurse thee, knave...!^n^xI shall smite thee down myself!^n^xThou shan't see^n^r"
            + self.get_checks_boss_name("Mithra", world)
            + "^p nor ^r"
            + self.get_checks_boss_name("Samael", world)
            + "^p!",
            "MITUKETANODA",
        )
        ...
        f033_mot_proc = f033_obj.getProcIndexByLabel(
            "024_moto_on"
        )  # Change mot model to new boss
        f033_mot_insts, f033_mot_labels = f033_obj.getProcInstructionsLabelsByIndex(
            f033_mot_proc
        )
        ...
        f033_mot_insts[90] = inst(
            "PUSHIS", self.get_checks_boss_id("Mot", world)
        )  # mot is loaded 4 times, unsure which is correct
        f033_obj.changeProcByIndex(f033_mot_insts, f033_mot_labels, f033_mot_proc)
        f033_obj.changeNameByLookup(
            "Mot", self.get_checks_boss_name("Mot", world, immersive=True)
        )

        f033_mada_rwms = f033_obj.appendMessage(
            self.get_reward_str("Mada", world), "MADA_RWMS"
        )
        ...
        f033_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Mot", world), "MOT_REWARD"), 0xD
        )

        # Flag insertion for Mot after line 44. Need to move labels.
        f033_18_proc = f033_obj.getProcIndexByLabel("018_start")
        f033_18_insts, f033_18_labels = f033_obj.getProcInstructionsLabelsByIndex(
            f033_18_proc
        )
        ...
        f033_29_labels = [assembler.label("MITHRA_FOUGHT", 17)]
        if not config_settings.longest_cutscenes:
            f033_obj.changeProcByIndex(f033_29_insts, f033_29_labels, f033_29_proc)
        else:
            f033_29_insts, f033_29_labels = f033_obj.getProcInstructionsLabelsByIndex(
                f033_29_proc
            )
            f033_29_insts[30] = inst("PUSHIS", self.get_checks_boss_id("Mithra", world))
            f033_obj.changeNameByLookup(
                "Mithra", self.get_checks_boss_name("Mithra", world, immersive=True)
            )
            f033_obj.changeProcByIndex(f033_29_insts, f033_29_labels, f033_29_proc)
        f033_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Mithra", world), "MITHRA_REWARD"),
            0x2D,
        )

        f033_29_start_proc = f033_obj.getProcIndexByLabel("029_start")  # Mithra reward
        f033_29_start_insts, f033_29_start_labels = (
            f033_obj.getProcInstructionsLabelsByIndex(f033_29_start_proc)
        )
        f033_29_start_insts = (
            f033_29_start_insts[0:28]
            + self.get_flag_reward_insts("Mithra", world)
            + f033_29_start_insts[28:]
        )
        f033_29_start_labels[0].label_offset = f033_29_start_labels[
            0
        ].label_offset + len(self.get_flag_reward_insts("Mithra", world))
        f033_29_start_labels[1].label_offset = f033_29_start_labels[
            1
        ].label_offset + len(self.get_flag_reward_insts("Mithra", world))
        f033_obj.changeProcByIndex(
            f033_29_start_insts, f033_29_start_labels, f033_29_start_proc
        )

        f033_samael_rwms = f033_obj.appendMessage(
            self.get_reward_str("Samael", world), "SAMAEL_RWMS"
        )
        ...
        e674_labels = [assembler.label("SAMAEL_DEFEATED", 13)]
        if config_settings.shortest_cutscenes:
            ...
            e674_main_insts, e674_main_labels = (
                e674_obj.getProcInstructionsLabelsByIndex(e674_main_proc)
            )
            ...
            for l in e674_main_labels:
                if l.label_offset > precut:
                    ...
                    if l.label_offset < 1:
                        l.label_offset = 1
            e674_main_insts = (
                e674_main_insts[:precut]
                + e674_main_insts[postcut:precut2]
                + e674_main_insts[postcut2:]
            )
            e674_obj.changeProcByIndex(
                e674_main_insts, e674_main_labels, e674_main_proc
            )

            e674_10_proc = e674_obj.getProcIndexByLabel("e674_010")
            e674_10_insts, e674_10_labels = e674_obj.getProcInstructionsLabelsByIndex(
                e674_10_proc
            )
            ...
            e674_100_insts, e674_100_labels = e674_obj.getProcInstructionsLabelsByIndex(
                e674_100_proc
            )
            e674_100_insts[52] = inst(
                "PUSHIS", self.get_checks_boss_id("Samael", world)
            )  # Load Samael Model
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e674"], BytesIO(bytes(e674_obj.toBytes()))
        )
        ...
        if (
            config_settings.longest_cutscenes or config_settings.shijima
        ):  # Model swap for long cutscenes or shijima
            if not config_settings.vanilla_tok:
                e681_20_proc = e681_obj.getProcIndexByLabel("e681_020")
                e681_20_insts, e681_20_labels = (
                    e681_obj.getProcInstructionsLabelsByIndex(e681_20_proc)
                )
                ...
                e681_30_insts, e681_30_labels = (
                    e681_obj.getProcInstructionsLabelsByIndex(e681_30_proc)
                )
                ...
                e681_80_insts, e681_80_labels = (
                    e681_obj.getProcInstructionsLabelsByIndex(e681_80_proc)
                )
                ...
                e681_main_insts, e681_main_labels = (
                    e681_obj.getProcInstructionsLabelsByIndex(e681_main_proc)
                )
                e681_main_insts[24] = inst(
                    "PUSHIS", self.get_checks_boss_id("Ahriman", world)
                )
                ...
                e681_main_insts = (
                    e681_main_insts[:insert_index]
                    + e681_main_insert_insts
                    + e681_main_insts[insert_index:]
                )
                e681_obj.changeProcByIndex(
                    e681_main_insts, e681_main_labels, e681_main_proc
                )
                e681_obj.changeNameByLookup(
                    "Ahriman",
                    self.get_checks_boss_name("Ahriman", world, immersive=True),
                )
        else:  # Remove cutscene
            e681_obj.changeProcByIndex(e681_insts, [], 0)
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e681"], BytesIO(bytes(e681_obj.toBytes()))
        )
        ...
        if config_settings.longest_cutscenes:
            if not config_settings.vanilla_tok:
                ...
                e678_main_insts, e678_main_labels = (
                    e678_obj.getProcInstructionsLabelsByIndex(e678_main_proc)
                )
                e678_main_insts[24] = inst(
                    "PUSHIS", self.get_checks_boss_id("Ahriman", world)
                )
                ...
                e678_main_insts = (
                    e678_main_insts[:89] + e678_main_insert_insts + e678_main_insts[89:]
                )
                e678_obj.changeProcByIndex(
                    e678_main_insts, e678_main_labels, e678_main_proc
                )
                e678_obj.changeNameByLookup(
                    "Hikawa",
                    self.get_checks_boss_name("Ahriman", world, immersive=True),
                )
        else:
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e678"], BytesIO(bytes(e678_obj.toBytes()))
        )
        ...
        # Ahriman hint
        f032_obj.changeMessageByIndex(
            assembler.message(
                "I saw an incredible demon up ahead.^nIt's big! It's red!^n^xIt's ^r"
                + self.get_checks_boss_name("Ahriman", world)
                + "^p!",
                "F032_MANE07",
            ),
            0x76,
        ...
        if not config_settings.musubi:  # Don't fight Noah in Musubi
            if config_settings.longest_cutscenes:  # Model swap
                if not config_settings.vanilla_tok:
                    ...
                    e680_10_insts, e680_10_labels = (
                        e680_obj.getProcInstructionsLabelsByIndex(e680_10_proc)
                    )
                    e680_10_insts[40] = inst("PUSHIS", 0x0)
                    e680_obj.changeProcByIndex(
                        e680_10_insts, e680_10_labels, e680_10_proc
                    )
                    e680_main_proc = e680_obj.getProcIndexByLabel("e680_main")
                    e680_main_insts, e680_main_labels = (
                        e680_obj.getProcInstructionsLabelsByIndex(e680_main_proc)
                    )
                    e680_main_insts[44] = inst(
                        "PUSHIS", self.get_checks_boss_id("Noah", world)
                    )
                    ...
                    e680_main_insts = (
                        e680_main_insts[:insert_index]
                        + e680_main_insert_insts
                        + e680_main_insts[insert_index:]
                    )
                    e680_obj.changeProcByIndex(
                        e680_main_insts, e680_main_labels, e680_main_proc
                    )
                    e680_obj.changeNameByIndex(
                        self.get_checks_boss_name("Noah", world, immersive=True), 0
                    )
            else:  # Remove cutscene
                e680_obj.changeProcByIndex(e680_insts, [], 0)
            self.dds3.add_new_file(
                custom_vals.SCRIPT_OBJ_PATH["e680"], BytesIO(bytes(e680_obj.toBytes()))
            )
        ...
        if config_settings.longest_cutscenes:
            if not config_settings.vanilla_tok:
                ...
                e677_main_insts, e677_main_labels = (
                    e677_obj.getProcInstructionsLabelsByIndex(e677_main_proc)
                )
                e677_main_insts[20] = inst(
                    "PUSHIS", self.get_checks_boss_id("Noah", world)
                )
                e677_main_insts[21] = inst("PUSHIS", 0x6)
                e677_main_insts[22] = inst("COMM", 0x15)
                e677_main_insts[59] = inst("PUSHIS", 0x2)
                e677_obj.changeProcByIndex(
                    e677_main_insts, e677_main_labels, e677_main_proc
                )
                e677_obj.changeNameByIndex(
                    self.get_checks_boss_name("Noah", world, immersive=True), 0
                )
        else:
            e677_main_insts = [
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e677"], BytesIO(bytes(e677_obj.toBytes()))
        )
        ...
        f036_12_insts, f036_12_labels = f036_obj.getProcInstructionsLabelsByIndex(
            f036_12_proc
        )
        ...
        f036_13_labels = copy.deepcopy(
            f036_12_labels
        )  # Need to deepcopy the labels because changeProcByIndex changes the label offsets from relative to absolute, and I didn't think of this situation when I wrote the assembler.
        ...
        # Noah hint
        if not config_settings.vanilla_tok:
            f036_obj.changeMessageByIndex(
                assembler.message(
                    "............^n^xI see......^n^xThen, go in. ^r"
                    + self.get_checks_boss_name("Noah", world)
                    + "^p^nis waiting...",
                    "F036_SINEN03_YES",
                ),
                0x32,
        ...
        if config_settings.longest_cutscenes:
            if not config_settings.vanilla_tok:
                ...
                e682_30_insts, e682_30_labels = (
                    e682_obj.getProcInstructionsLabelsByIndex(e682_30_proc)
                )
                ...
                e682_50_insts, e682_50_labels = (
                    e682_obj.getProcInstructionsLabelsByIndex(e682_50_proc)
                )
                ...
                e682_main_insts, e682_main_labels = (
                    e682_obj.getProcInstructionsLabelsByIndex(e682_main_proc)
                )
                e682_main_insts[12] = inst(
                    "PUSHIS", self.get_checks_boss_id("Baal Avatar", world)
                )
                e682_main_insts[13] = inst("PUSHIS", 0x6)
                e682_main_insts[14] = inst("COMM", 0x15)
                e682_obj.changeProcByIndex(
                    e682_main_insts, e682_main_labels, e682_main_proc
                )
                e682_obj.changeNameByIndex(
                    self.get_checks_boss_name("Baal Avatar", world, immersive=True), 0
                )
        else:
            e682_obj.changeProcByIndex(e682_insts, [], 0)
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e682"], BytesIO(bytes(e682_obj.toBytes()))
        )
        ...
        if config_settings.longest_cutscenes:
            if not config_settings.vanilla_tok:
                ...
                e679_main_insts, e679_main_labels = (
                    e679_obj.getProcInstructionsLabelsByIndex(e679_main_proc)
                )
                e679_main_insts[20] = inst(
                    "PUSHIS", self.get_checks_boss_id("Baal Avatar", world)
                )
                e679_main_insts[21] = inst("PUSHIS", 0x6)
                e679_main_insts[22] = inst("COMM", 0x15)
                e679_obj.changeProcByIndex(
                    e679_main_insts, e679_main_labels, e679_main_proc
                )
                e679_10_proc = e679_obj.getProcIndexByLabel("e679_010")
                e679_10_insts, e679_10_labels = (
                    e679_obj.getProcInstructionsLabelsByIndex(e679_10_proc)
                )
                e679_10_insts[15] = inst("PUSHIS", 0x2)
                e679_obj.changeProcByIndex(e679_10_insts, e679_10_labels, e679_10_proc)
                e679_obj.changeNameByIndex(
                    self.get_checks_boss_name("Baal Avatar", world, immersive=True), 0
                )
        else:
            e679_main_insts = [
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e679"], BytesIO(bytes(e679_obj.toBytes()))
        )
        ...
        f037_19_insts, f037_19_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_19_proc
        )
        ...
        f037_20_insts, f037_20_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_20_proc
        )
        ...
        f037_21_insts, f037_21_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_21_proc
        )
        ...
        f037_23_insts, f037_23_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_23_proc
        )
        ...
        f037_obj.changeMessageByIndex(
            assembler.message(
                "You can see ^r"
                + self.get_checks_boss_name("Thor 2", world)
                + "^p^nat the end of the hall.",
                "THOR00",
            ),
            0x2,
        )
        f037_thor_name_id = f037_obj.sections[3].messages[0x5].name_id
        f037_thor_message = assembler.message(
            "...I am ^r"
            + self.get_checks_boss_name("Thor 2", world, immersive=True)
            + "^p. Our paths have not^ncrossed since the fall of the Mantra.",
            "THOR03",
        )
        f037_thor_message.name_id = f037_thor_name_id
        f037_obj.changeMessageByIndex(f037_thor_message, 0x5)
        f037_obj.changeMessageByIndex(
            assembler.message(self.get_reward_str("Thor 2", world), "THOR04"), 0x10
        )

        f037_thorafter_proc = f037_obj.getProcIndexByLabel(
            "007_THOR_AFTER"
        )  # Change Thor 2 model to new boss before and after fight
        f037_thorafter_insts, f037_thorafter_labels = (
            f037_obj.getProcInstructionsLabelsByIndex(f037_thorafter_proc)
        )
        f037_thorafter_insts[8] = inst(
            "PUSHIS", self.get_checks_boss_id("Thor 2", world)
        )
        if not config_settings.longest_cutscenes:
            precut = 4
            postcut = 148
            ...
            for l in f037_thorafter_labels:
                if l.label_offset > precut:
                    ...
                    if l.label_offset < 1:
                        l.label_offset = 1
            f037_thorafter_insts = (
                f037_thorafter_insts[:precut] + f037_thorafter_insts[postcut:]
            )
        f037_obj.changeProcByIndex(
            f037_thorafter_insts, f037_thorafter_labels, f037_thorafter_proc
        )

        f037_thor_proc = f037_obj.getProcIndexByLabel("007_01eve_05")
        f037_thor_insts, f037_thor_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_thor_proc
        )
        if not config_settings.longest_cutscenes:
            precut = 7
            ...
            f037_obj.changeProcByIndex(
                f037_thor_insts, f037_thor_labels, f037_thor_proc
            )
        elif not config_settings.vanilla_tok:
            f037_thor_insts[18] = inst(
                "PUSHIS", self.get_checks_boss_id("Thor 2", world)
            )
        ...
        f037_obj.changeNameByLookup(
            "Thor", self.get_checks_boss_name("Thor 2", world, immersive=True)
        )

        # Baal hint for normal and Yosuga
        if not config_settings.vanilla_tok:
            f037_dominion_name_id = f037_obj.sections[3].messages[0x4D].name_id
            f037_dominion_message1 = assembler.message(
                "If it isn't the Demi-fiend...^n^xWe've been waiting for you,^nand that stone you have in^nyour hands.^xLady ^r"
                + self.get_checks_boss_name("Baal Avatar", world)
                + "^p is waiting for^nyou up ahead. Please go on through.",
                "F037_DEVIL03_01",
            )
            f037_dominion_message1.name_id = f037_dominion_name_id
            f037_obj.changeMessageByIndex(f037_dominion_message1, 0x4D)
            f037_dominion_message2 = assembler.message(
                "If it isn't the Demi-fiend...^n^xI'm surprised you have the nerve to^neven see us!^n^xLady ^r"
                + self.get_checks_boss_name("Baal Avatar", world)
                + "^p^nis up ahead.^n^xYosuga shall take that stone^nyou have in your hands.^n...And your life.",
                "F037_DEVIL03_02",
            )
        ...
        f037_27_proc = f037_obj.getProcIndexByLabel(
            "027_start"
        )  # Auto-insert stones if you have all 3
        f037_27_insts, f037_27_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_27_proc
        )
        ...
        f037_27_auto_stone_label_offset = len(f037_27_insts) + len(
            f037_27_insert_insts_autostonecheck
        )
        f037_27_labels.append(
            assembler.label("AUTO_INSERT_stone", f037_27_auto_stone_label_offset)
        )
        f037_obj.changeMessageByIndex(
            assembler.message("Stones automatically inserted.", "AUTO_stone"), 0x1F
        )
        f037_27_04_proc = f037_obj.getProcIndexByLabel(
            "027_01eve_04"
        )  # Auto-insert stones if you have all 3
        f037_27_04_insts, f037_27_04_labels = f037_obj.getProcInstructionsLabelsByIndex(
            f037_27_04_proc
        )
        f037_27_unlock_kagutsuchi_insts = f037_27_04_insts[
            233:281
        ]  # Code that unlocks the platform to kagutsuchi
        f037_27_insert_insts_autostone_do = (
            [
                ...
                inst(
                    "PUSHIS", 0x66B
                ),  # Remove all stones from inventory and set their inserted flags
                inst("COMM", 0x8),
                inst("PUSHIS", 0x66C),
                inst("COMM", 0x8),
                inst("PUSHIS", 0x66D),
            ...
            + [inst("GOTO", f037_27_auto_stone_label_index - 1), inst("END")]
        )  # Add goto end

        f037_27_labels[-2].label_offset += len(
            f037_27_insert_insts_autostone_do
        )  # Fix former end label
        f037_27_insts = (
            f037_27_insts[:57]
            + f037_27_insert_insts_autostonecheck
            + f037_27_insts[57:]
        ...
        f038_zouchou_proc = f038_obj.getProcIndexByLabel(
            "001_01eve_10"
        )  # Zoucho model swap
        f038_zouchou_insts, f038_zouchou_labels = (
            f038_obj.getProcInstructionsLabelsByIndex(f038_zouchou_proc)
        )
        f038_zouchou_insts[116] = inst(
            "PUSHIS", self.get_checks_boss_id("Zouchou", world)
        )
        f038_jikoku_proc = f038_obj.getProcIndexByLabel(
            "001_01eve_11"
        )  # Jikoku model swap
        f038_jikoku_insts, f038_jikoku_labels = (
            f038_obj.getProcInstructionsLabelsByIndex(f038_jikoku_proc)
        )
        f038_jikoku_insts[116] = inst(
            "PUSHIS", self.get_checks_boss_id("Jikoku", world)
        )
        f038_koumoku_proc = f038_obj.getProcIndexByLabel(
            "001_01eve_12"
        )  # Koumoku model swap
        f038_koumoku_insts, f038_koumoku_labels = (
            f038_obj.getProcInstructionsLabelsByIndex(f038_koumoku_proc)
        )
        f038_koumoku_insts[116] = inst(
            "PUSHIS", self.get_checks_boss_id("Koumoku", world)
        )
        f038_bishamon_proc = f038_obj.getProcIndexByLabel(
            "001_01eve_13"
        )  # Bishamon model swap
        f038_bishamon_insts, f038_bishamon_labels = (
            f038_obj.getProcInstructionsLabelsByIndex(f038_bishamon_proc)
        )
        f038_bishamon_insts[116] = inst(
            "PUSHIS", self.get_checks_boss_id("Bishamon 2", world)
        )
        ...
        if config_settings.shortest_cutscenes:  # Remove pillar lowering cutscenes
            ...
            postcut = 210
            diff = postcut - precut
            f038_zouchou_insert_insts = [inst("PUSHIS", 0x713), inst("COMM", 0x8)]
            f038_jikoku_insert_insts = [inst("PUSHIS", 0x714), inst("COMM", 0x8)]
            f038_koumoku_insert_insts = [inst("PUSHIS", 0x712), inst("COMM", 0x8)]
            f038_bishamon_insert_insts = [inst("PUSHIS", 0x711), inst("COMM", 0x8)]
            ...
            f038_zouchou_insts = (
                f038_zouchou_insts[:precut]
                + f038_zouchou_insert_insts
                + f038_zouchou_insts[postcut:]
            )
            f038_jikoku_insts = (
                f038_jikoku_insts[:precut]
                + f038_jikoku_insert_insts
                + f038_jikoku_insts[postcut:]
            )
            f038_bishamon_insts = (
                f038_bishamon_insts[:precut]
                + f038_bishamon_insert_insts
                + f038_bishamon_insts[postcut:]
            )
            ...
            f038_koumoku_insts = (
                f038_koumoku_insts[:precut]
                + f038_koumoku_insert_insts
                + f038_koumoku_insts[postcut:]
            )

        f038_obj.changeProcByIndex(
            f038_zouchou_insts, f038_zouchou_labels, f038_zouchou_proc
        )
        f038_obj.changeProcByIndex(
            f038_jikoku_insts, f038_jikoku_labels, f038_jikoku_proc
        )
        f038_obj.changeProcByIndex(
            f038_bishamon_insts, f038_bishamon_labels, f038_bishamon_proc
        )
        f038_obj.changeProcByIndex(
            f038_koumoku_insts, f038_koumoku_labels, f038_koumoku_proc
        )

        f038_obj.changeNameByLookup(
            "Zouchou", self.get_checks_boss_name("Zouchou", world, immersive=True)
        )
        f038_obj.changeNameByLookup(
            "Jikoku", self.get_checks_boss_name("Jikoku", world, immersive=True)
        )
        f038_obj.changeNameByLookup(
            "Koumoku", self.get_checks_boss_name("Koumoku", world, immersive=True)
        )
        f038_obj.changeNameByLookup(
            "Bishamon", self.get_checks_boss_name("Bishamon 2", world, immersive=True)
        )

        # Shorten masakados
        f038_masakados_proc = f038_obj.getProcIndexByLabel("002_start")
        f038_masakados_insts, f038_masakados_labels = (
            f038_obj.getProcInstructionsLabelsByIndex(f038_masakados_proc)
        )
        ...
        for l in f038_masakados_labels:
            if l.label_offset > precut:
                ...
                if l.label_offset < 1:
                    l.label_offset = 1
        f038_masakados_insts = (
            f038_masakados_insts[:precut]
            + f038_masakados_insert_insts
            + f038_masakados_insts[postcut:]
        )
        f038_obj.changeProcByIndex(
            f038_masakados_insts, f038_masakados_labels, f038_masakados_proc
        )
        ...
        e705_main_insts, e705_main_labels = e705_obj.getProcInstructionsLabelsByIndex(
            e705_main_proc
        )
        if not config_settings.vanilla_tok:
            e705_main_insts[35] = inst(
                "PUSHIS", self.get_checks_boss_id("Kagutsuchi", world)
            )
            e705_main_insts[36] = inst("PUSHIS", 0x6)
            e705_main_insts[37] = inst("COMM", 0x15)
            e705_main_insts[45] = inst(
                "PUSHIS", self.get_checks_boss_id("Kagutsuchi", world)
            )
            e705_main_insts[46] = inst("PUSHIS", 0x6)
            e705_main_insts[47] = inst("COMM", 0x15)
            e705_obj.changeNameByLookup(
                "Voice", self.get_checks_boss_name("Kagutsuchi", world, immersive=True)
            )
            e705_obj.changeNameByLookup(
                "Kagutsuchi",
                self.get_checks_boss_name("Kagutsuchi", world, immersive=True),
            )
        ...
        e705_10_insts, e705_10_labels = e705_obj.getProcInstructionsLabelsByIndex(
            e705_10_proc
        )
        ...
        for l in e705_10_labels:
            if l.label_offset > precut:
                ...
                if l.label_offset < 1:
                    l.label_offset = 1
        e705_10_insts = (
            e705_10_insts[:precut] + e705_10_insert_insts + e705_10_insts[postcut:]
        )
        e705_obj.changeProcByIndex(e705_10_insts, e705_10_labels, e705_10_proc)

        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e705"], BytesIO(bytes(e705_obj.toBytes()))
        )
        ...
        e726_main_insts, e726_main_labels = e726_obj.getProcInstructionsLabelsByIndex(
            e726_main_proc
        )
        precut = 11
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e726"], BytesIO(bytes(e726_obj.toBytes()))
        )
        ...
        f040_kalpa1_tunnel_insts, f040_kalpa1_tunnel_labels = (
            f040_obj.getProcInstructionsLabelsByIndex(f040_kalpa1_tunnel_proc)
        )
        ...
        f040_kalpa1_tunnel_insts = (
            f040_kalpa1_tunnel_insts[0:64]
            + f040_kalpa1_tunnel_insert_insts
            + f040_kalpa1_tunnel_insts[66:-1]
            + [inst("END")]
        )
        for l in f040_kalpa1_tunnel_labels:
            if l.label_offset > 64:
                l.label_offset += 4
        f040_obj.changeProcByIndex(
            f040_kalpa1_tunnel_insts, f040_kalpa1_tunnel_labels, f040_kalpa1_tunnel_proc
        )

        f040_kalpa2_tunnel_proc = f040_obj.getProcIndexByLabel("001_01eve_03")
        f040_kalpa2_tunnel_insts, f040_kalpa2_tunnel_labels = (
            f040_obj.getProcInstructionsLabelsByIndex(f040_kalpa2_tunnel_proc)
        )
        ...
        f040_kalpa2_tunnel_insts = (
            f040_kalpa2_tunnel_insts[0:64]
            + f040_kalpa2_tunnel_insert_insts
            + f040_kalpa2_tunnel_insts[66:-1]
            + [inst("END")]
        )
        for l in f040_kalpa2_tunnel_labels:
            if l.label_offset > 64:
                l.label_offset += 4
        f040_obj.changeProcByIndex(
            f040_kalpa2_tunnel_insts, f040_kalpa2_tunnel_labels, f040_kalpa2_tunnel_proc
        )

        f040_kalpa3_tunnel_proc = f040_obj.getProcIndexByLabel("001_01eve_04")
        f040_kalpa3_tunnel_insts, f040_kalpa3_tunnel_labels = (
            f040_obj.getProcInstructionsLabelsByIndex(f040_kalpa3_tunnel_proc)
        )
        ...
        f040_kalpa3_tunnel_insts = (
            f040_kalpa3_tunnel_insts[0:64]
            + f040_kalpa3_tunnel_insert_insts
            + f040_kalpa3_tunnel_insts[66:-1]
            + [inst("END")]
        )
        for l in f040_kalpa3_tunnel_labels:
            if l.label_offset > 64:
                l.label_offset += 4
        f040_obj.changeProcByIndex(
            f040_kalpa3_tunnel_insts, f040_kalpa3_tunnel_labels, f040_kalpa3_tunnel_proc
        )

        f040_kalpa4_tunnel_proc = f040_obj.getProcIndexByLabel("001_01eve_05")
        f040_kalpa4_tunnel_insts, f040_kalpa4_tunnel_labels = (
            f040_obj.getProcInstructionsLabelsByIndex(f040_kalpa4_tunnel_proc)
        )
        ...
        f040_kalpa4_tunnel_insts = (
            f040_kalpa4_tunnel_insts[0:64]
            + f040_kalpa4_tunnel_insert_insts
            + f040_kalpa4_tunnel_insts[66:-1]
            + [inst("END")]
        )
        for l in f040_kalpa4_tunnel_labels:
            if l.label_offset > 64:
                l.label_offset += 4
        f040_obj.changeProcByIndex(
            f040_kalpa4_tunnel_insts, f040_kalpa4_tunnel_labels, f040_kalpa4_tunnel_proc
        )

        f040_kalpa5_tunnel_proc = f040_obj.getProcIndexByLabel("001_01eve_06")
        f040_kalpa5_tunnel_insts, f040_kalpa5_tunnel_labels = (
            f040_obj.getProcInstructionsLabelsByIndex(f040_kalpa5_tunnel_proc)
        )
        ...
        f040_kalpa5_tunnel_insts = (
            f040_kalpa5_tunnel_insts[0:64]
            + f040_kalpa5_tunnel_insert_insts
            + f040_kalpa5_tunnel_insts[66:-1]
            + [inst("END")]
        )
        for l in f040_kalpa5_tunnel_labels:
            if l.label_offset > 64:
                l.label_offset += 4
        f040_obj.changeProcByIndex(
            f040_kalpa5_tunnel_insts, f040_kalpa5_tunnel_labels, f040_kalpa5_tunnel_proc
        )

        f040_door_proc = f040_obj.getProcIndexByLabel("001_door")
        f040_door_insts, f040_door_labels = f040_obj.getProcInstructionsLabelsByIndex(
            f040_door_proc
        )
        precut = 47
        postcut = 71
        ...
        f041_kalpa2_tunnel_insts, f041_kalpa2_tunnel_labels = (
            f041_obj.getProcInstructionsLabelsByIndex(f041_kalpa2_tunnel_proc)
        )
        ...
        f041_kalpa2_tunnel_insts = (
            f041_kalpa2_tunnel_insts[0:60]
            + f041_kalpa2_tunnel_insert_insts
            + f041_kalpa2_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f041_kalpa2_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f041_obj.changeProcByIndex(
            f041_kalpa2_tunnel_insts, f041_kalpa2_tunnel_labels, f041_kalpa2_tunnel_proc
        )

        f041_lobby_tunnel_proc = f041_obj.getProcIndexByLabel("001_01eve_02")
        f041_lobby_tunnel_insts, f041_lobby_tunnel_labels = (
            f041_obj.getProcInstructionsLabelsByIndex(f041_lobby_tunnel_proc)
        )
        ...
        f041_lobby_tunnel_insts = (
            f041_lobby_tunnel_insts[0:60]
            + f041_lobby_tunnel_insert_insts
            + f041_lobby_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f041_lobby_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f041_obj.changeProcByIndex(
            f041_lobby_tunnel_insts, f041_lobby_tunnel_labels, f041_lobby_tunnel_proc
        )

        f041_door_proc = f041_obj.getProcIndexByLabel("002_door")
        f041_door_insts, f041_door_labels = f041_obj.getProcInstructionsLabelsByIndex(
            f041_door_proc
        )
        precut = 42
        postcut = 66
        ...
        f041_switch_insts, f041_switch_labels = (
            f041_obj.getProcInstructionsLabelsByIndex(f041_switch_proc)
        )
        precut = 108
        postcut = 137
        diff = postcut - precut
        ...
        f041_obj.changeProcByIndex(
            f041_switch_insts, f041_switch_labels, f041_switch_proc
        )

        f041_menorah_proc = f041_obj.getProcIndexByLabel(
            "001_01eve_01"
        )  # Shorten placing menorah
        f041_menorah_insts, f041_menorah_labels = (
            f041_obj.getProcInstructionsLabelsByIndex(f041_menorah_proc)
        )
        ...
        f041_menorah_insts = (
            f041_menorah_insts[:precut]
            + f041_menorah_insts[postcut:precut2]
            + f041_menorah_insts[postcut2:precut3]
            + f041_menorah_insts[postcut3:precut4]
            + f041_menorah_insts[postcut4:]
        )
        f041_obj.changeProcByIndex(
            f041_menorah_insts, f041_menorah_labels, f041_menorah_proc
        )
        ...
        f042_kalpa1_tunnel_insts, f042_kalpa1_tunnel_labels = (
            f042_obj.getProcInstructionsLabelsByIndex(f042_kalpa1_tunnel_proc)
        )
        ...
        f042_kalpa1_tunnel_insts = (
            f042_kalpa1_tunnel_insts[0:60]
            + f042_kalpa1_tunnel_insert_insts
            + f042_kalpa1_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f042_kalpa1_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f042_obj.changeProcByIndex(
            f042_kalpa1_tunnel_insts, f042_kalpa1_tunnel_labels, f042_kalpa1_tunnel_proc
        )

        f042_lobby_tunnel_proc = f042_obj.getProcIndexByLabel("001_01eve_03")
        f042_lobby_tunnel_insts, f042_lobby_tunnel_labels = (
            f042_obj.getProcInstructionsLabelsByIndex(f042_lobby_tunnel_proc)
        )
        ...
        f042_lobby_tunnel_insts = (
            f042_lobby_tunnel_insts[0:60]
            + f042_lobby_tunnel_insert_insts
            + f042_lobby_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f042_lobby_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f042_obj.changeProcByIndex(
            f042_lobby_tunnel_insts, f042_lobby_tunnel_labels, f042_lobby_tunnel_proc
        )

        f042_kalpa3_tunnel_proc = f042_obj.getProcIndexByLabel("002_01eve_02")
        f042_kalpa3_tunnel_insts, f042_kalpa3_tunnel_labels = (
            f042_obj.getProcInstructionsLabelsByIndex(f042_kalpa3_tunnel_proc)
        )
        ...
        f042_kalpa3_tunnel_insts = (
            f042_kalpa3_tunnel_insts[0:60]
            + f042_kalpa3_tunnel_insert_insts
            + f042_kalpa3_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f042_kalpa3_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f042_obj.changeProcByIndex(
            f042_kalpa3_tunnel_insts, f042_kalpa3_tunnel_labels, f042_kalpa3_tunnel_proc
        )

        # Apoc stone hint
        f042_arahabaki_name_id = f042_obj.sections[3].messages[0x84].name_id
        f042_arahabaki_message = assembler.message(
            "The riders are not here.^n^xThey left while sayeth their stone^nis at ^g"
            + self.get_flag_reward_location_string(0x3F4, world)
            + "^p.",
            "MSG_F42_A2_AKUMA01",
        )
        ...
        if config_settings.menorah_groups:  # Menorah hint
            f042_obj.changeMessageByIndex(
                assembler.message(
                    "> Retrieve these candelabra^nfrom ^g"
                    + self.get_flag_reward_location_string(0x3E7, world)
                    + "^p.",
                    "MSG_001_2",
                ),
                0x6,
        ...
        f042_door_insts, f042_door_labels = f042_obj.getProcInstructionsLabelsByIndex(
            f042_door_proc
        )
        precut = 42
        postcut = 66
        ...
        f042_menorah_proc = f042_obj.getProcIndexByLabel(
            "001_01eve_01"
        )  # Shorten placing menorahs
        f042_menorah_insts, f042_menorah_labels = (
            f042_obj.getProcInstructionsLabelsByIndex(f042_menorah_proc)
        )
        ...
        f042_menorah_insts = (
            f042_menorah_insts[:precut]
            + f042_menorah_insts[postcut:precut2]
            + f042_menorah_insts[postcut2:precut3]
            + f042_menorah_insts[postcut3:precut4]
            + f042_menorah_insts[postcut4:precut5]
            + f042_menorah_insts[postcut5:]
        )
        f042_obj.changeProcByIndex(
            f042_menorah_insts, f042_menorah_labels, f042_menorah_proc
        )
        ...
        e711_main_insts, e711_main_labels = e711_obj.getProcInstructionsLabelsByIndex(
            e711_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e711"], BytesIO(bytes(e711_obj.toBytes()))
        )

        e712_obj = self.get_script_obj_by_name("e712")
        e712_main_proc = e712_obj.getProcIndexByLabel("e712_main")
        e712_main_insts, e712_main_labels = e712_obj.getProcInstructionsLabelsByIndex(
            e712_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e712"], BytesIO(bytes(e712_obj.toBytes()))
        )

        e713_obj = self.get_script_obj_by_name("e713")
        e713_main_proc = e713_obj.getProcIndexByLabel("e713_main")
        e713_main_insts, e713_main_labels = e713_obj.getProcInstructionsLabelsByIndex(
            e713_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e713"], BytesIO(bytes(e713_obj.toBytes()))
        )

        e714_obj = self.get_script_obj_by_name("e714")
        e714_main_proc = e714_obj.getProcIndexByLabel("e714_main")
        e714_main_insts, e714_main_labels = e714_obj.getProcInstructionsLabelsByIndex(
            e714_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e714"], BytesIO(bytes(e714_obj.toBytes()))
        )

        e715_obj = self.get_script_obj_by_name("e715")
        e715_main_proc = e715_obj.getProcIndexByLabel("e715_main")
        e715_main_insts, e715_main_labels = e715_obj.getProcInstructionsLabelsByIndex(
            e715_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e715"], BytesIO(bytes(e715_obj.toBytes()))
        )
        ...
        e729_main_insts, e729_main_labels = e729_obj.getProcInstructionsLabelsByIndex(
            e729_main_proc
        )
        if not config_settings.longest_cutscenes:  # Skip cutscene
            ...
            e729_main_labels = [assembler.label("DANTE_FOUGHT", 13)]
            e729_obj.changeProcByIndex(
                e729_main_insts, e729_main_labels, e729_main_proc
            )
        else:  # Dante 2 model swap
            e729_20_proc = e729_obj.getProcIndexByLabel("e729_020")
            e729_20_insts, e729_20_labels = e729_obj.getProcInstructionsLabelsByIndex(
                e729_20_proc
            )
            ...
            e729_30_insts, e729_30_labels = e729_obj.getProcInstructionsLabelsByIndex(
                e729_30_proc
            )
            ...
            e729_obj.changeNameByLookup(
                "Dante", self.get_checks_boss_name("Dante 2", world, immersive=True)
            )
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e729"], BytesIO(bytes(e729_obj.toBytes()))
        )

        # Dante 2 reward
        f043_obj = self.get_script_obj_by_name("f043")
        f043_dante_callback_proc = f043_obj.getProcIndexByLabel("012_battle_aft")
        f043_dante_callback_insts, f043_dante_callback_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_dante_callback_proc)
        )
        if config_settings.menorah_groups:
            f043_dante_callback_insts = (
                f043_dante_callback_insts[0:8]
                + self.get_flag_reward_insts("Dante 2", world)
                + f043_dante_callback_insts[8:-1]
                + [inst("END")]
            )
        else:
        ...
        f043_obj.changeProcByIndex(
            f043_dante_callback_insts,
            f043_dante_callback_labels,
            f043_dante_callback_proc,
        )
        f043_obj.changeMessageByIndex(
            assembler.message(
                self.get_reward_str("Dante 2", world), "MSG_012_BATTLE_AFTER"
            ),
            0x25,
        )
        f043_obj.changeMessageByIndex(
            assembler.message(
                "You sense the presence of^n^r"
                + self.get_checks_boss_name("Dante 2", world)
                + "^p.",
                "FIRE_YURE",
            ),
            0x31,
        ...
        f043_door7_insts, f043_door7_labels = f043_obj.getProcInstructionsLabelsByIndex(
            f043_door7_proc
        )
        ...
        f043_door5_insts, f043_door5_labels = f043_obj.getProcInstructionsLabelsByIndex(
            f043_door5_proc
        )
        ...
        f043_kalpa2_tunnel_insts, f043_kalpa2_tunnel_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_kalpa2_tunnel_proc)
        )
        ...
        f043_kalpa2_tunnel_insts = (
            f043_kalpa2_tunnel_insts[0:60]
            + f043_kalpa2_tunnel_insert_insts
            + f043_kalpa2_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f043_kalpa2_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f043_obj.changeProcByIndex(
            f043_kalpa2_tunnel_insts, f043_kalpa2_tunnel_labels, f043_kalpa2_tunnel_proc
        )

        f043_lobby_tunnel_proc = f043_obj.getProcIndexByLabel("001_01eve_03")
        f043_lobby_tunnel_insts, f043_lobby_tunnel_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_lobby_tunnel_proc)
        )
        ...
        f043_lobby_tunnel_insts = (
            f043_lobby_tunnel_insts[0:60]
            + f043_lobby_tunnel_insert_insts
            + f043_lobby_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f043_lobby_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f043_obj.changeProcByIndex(
            f043_lobby_tunnel_insts, f043_lobby_tunnel_labels, f043_lobby_tunnel_proc
        )

        f043_kalpa4_tunnel_proc = f043_obj.getProcIndexByLabel("002_01eve_02")
        f043_kalpa4_tunnel_insts, f043_kalpa4_tunnel_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_kalpa4_tunnel_proc)
        )
        ...
        f043_kalpa4_tunnel_insts = (
            f043_kalpa4_tunnel_insts[0:60]
            + f043_kalpa4_tunnel_insert_insts
            + f043_kalpa4_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f043_kalpa4_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f043_obj.changeProcByIndex(
            f043_kalpa4_tunnel_insts, f043_kalpa4_tunnel_labels, f043_kalpa4_tunnel_proc
        )

        f043_star_tunnel_proc = f043_obj.getProcIndexByLabel("029_01eve_01")
        f043_star_tunnel_insts, f043_star_tunnel_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_star_tunnel_proc)
        )
        ...
        f043_star_tunnel_insts = (
            f043_star_tunnel_insts[0:60]
            + f043_star_tunnel_insert_insts
            + f043_star_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f043_star_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f043_obj.changeProcByIndex(
            f043_star_tunnel_insts, f043_star_tunnel_labels, f043_star_tunnel_proc
        )
        ...
        if config_settings.menorah_groups:  # Menorah hint
            f043_obj.changeMessageByIndex(
                assembler.message(
                    "> Retrieve these candelabra^nfrom ^g"
                    + self.get_flag_reward_location_string(0x3E4, world)
                    + "^p.",
                    "MSG_001_2",
                ),
                0xC,
        ...
        f043_dante_trio_1_proc = f043_obj.getProcIndexByLabel(
            "007_01eve_01"
        )  # Activate all 3 switches at once
        f043_dante_trio_1_insts, f043_dante_trio_1_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_dante_trio_1_proc)
        )
        ...
        for l in f043_dante_trio_1_labels:
            if l.label_offset >= 112:
                l.label_offset += 4
        f043_dante_trio_1_insts = (
            f043_dante_trio_1_insts[:112]
            + f043_dante_trio_1_insert_insts
            + f043_dante_trio_1_insts[112:]
        )
        f043_obj.changeProcByIndex(
            f043_dante_trio_1_insts, f043_dante_trio_1_labels, f043_dante_trio_1_proc
        )

        f043_dante_trio_2_proc = f043_obj.getProcIndexByLabel(
            "007_01eve_02"
        )  # Activate all 3 switches at once
        f043_dante_trio_2_insts, f043_dante_trio_2_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_dante_trio_2_proc)
        )
        ...
        for l in f043_dante_trio_2_labels:
            if l.label_offset >= 112:
                l.label_offset += 4
        f043_dante_trio_2_insts = (
            f043_dante_trio_2_insts[:112]
            + f043_dante_trio_2_insert_insts
            + f043_dante_trio_2_insts[112:]
        )
        f043_obj.changeProcByIndex(
            f043_dante_trio_2_insts, f043_dante_trio_2_labels, f043_dante_trio_2_proc
        )

        f043_dante_trio_3_proc = f043_obj.getProcIndexByLabel(
            "007_01eve_03"
        )  # Activate all 3 switches at once
        f043_dante_trio_3_insts, f043_dante_trio_3_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_dante_trio_3_proc)
        )
        ...
        for l in f043_dante_trio_3_labels:
            if l.label_offset >= 112:
                l.label_offset += 4
        f043_dante_trio_3_insts = (
            f043_dante_trio_3_insts[:112]
            + f043_dante_trio_3_insert_insts
            + f043_dante_trio_3_insts[112:]
        )
        f043_obj.changeProcByIndex(
            f043_dante_trio_3_insts, f043_dante_trio_3_labels, f043_dante_trio_3_proc
        )

        f043_door_proc = f043_obj.getProcIndexByLabel(
            "002_door"
        )  # Shorten door opening
        f043_door_insts, f043_door_labels = f043_obj.getProcInstructionsLabelsByIndex(
            f043_door_proc
        )
        precut = 42
        postcut = 66
        ...
        f043_dante_door_1_proc = f043_obj.getProcIndexByLabel(
            "005_door_open"
        )  # Shorten door opening
        f043_dante_door_1_insts, f043_dante_door_1_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_dante_door_1_proc)
        )
        precut = 22
        postcut = 35
        diff = postcut - precut
        for l in f043_dante_door_1_labels:
            if l.label_offset > precut:
                l.label_offset -= diff
        f043_dante_door_1_insts = (
            f043_dante_door_1_insts[:precut] + f043_dante_door_1_insts[postcut:]
        )
        f043_obj.changeProcByIndex(
            f043_dante_door_1_insts, f043_dante_door_1_labels, f043_dante_door_1_proc
        )

        f043_dante_door_2_proc = f043_obj.getProcIndexByLabel(
            "007_door_open"
        )  # Shorten door opening
        f043_dante_door_2_insts, f043_dante_door_2_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_dante_door_2_proc)
        )
        precut = 22
        postcut = 35
        diff = postcut - precut
        for l in f043_dante_door_2_labels:
            if l.label_offset > precut:
                l.label_offset -= diff
        f043_dante_door_2_insts = (
            f043_dante_door_2_insts[:precut] + f043_dante_door_2_insts[postcut:]
        )
        f043_obj.changeProcByIndex(
            f043_dante_door_2_insts, f043_dante_door_2_labels, f043_dante_door_2_proc
        )

        f043_menorah_proc = f043_obj.getProcIndexByLabel(
            "001_01eve_01"
        )  # Shorten placing menorahs
        f043_menorah_insts, f043_menorah_labels = (
            f043_obj.getProcInstructionsLabelsByIndex(f043_menorah_proc)
        )
        ...
        f043_menorah_insts = (
            f043_menorah_insts[:precut]
            + f043_menorah_insts[postcut:precut2]
            + f043_menorah_insts[postcut2:precut3]
            + f043_menorah_insts[postcut3:precut4]
            + f043_menorah_insts[postcut4:precut5]
            + f043_menorah_insts[postcut5:precut6]
            + f043_menorah_insts[postcut6:]
        )
        f043_obj.changeProcByIndex(
            f043_menorah_insts, f043_menorah_labels, f043_menorah_proc
        )
        ...
        e749_main_insts, e749_main_labels = e749_obj.getProcInstructionsLabelsByIndex(
            e749_main_proc
        )
        if not config_settings.longest_cutscenes:
            ...
            e749_main_labels = [assembler.label("BELZ_FOUGHT", 15)]
            e749_obj.changeProcByIndex(
                e749_main_insts, e749_main_labels, e749_main_proc
            )
        else:
            e749_10_proc = e749_obj.getProcIndexByLabel("e749_010")
            e749_10_insts, e749_10_labels = e749_obj.getProcInstructionsLabelsByIndex(
                e749_10_proc
            )
            ...
            e749_main_insts[62] = inst(
                "PUSHIS", self.get_checks_boss_id("Beelzebub", world)
            )
            e749_main_insts[63] = inst("PUSHIS", 0x6)
            e749_main_insts[64] = inst("COMM", 0x15)
            e749_main_insts[67] = inst(
                "PUSHIS", self.get_checks_boss_id("Beelzebub", world, index=1)
            )
            ...
            e749_bubs_message = assembler.message(
                "...I am chief among those who fell^nfrom heaven.^n^xWith the angel of darkness,^nI lead the demons of chaos.^n^x...I am "
                + self.get_checks_boss_name("Beelzebub", world, immersive=True)
                + ", ruler of death^nand warden of souls.",
                "MSG_001_010",
            )
            e749_bubs_message.name_id = e749_bubs_name_id
            e749_obj.changeMessageByIndex(e749_bubs_message, 0x2)
            e749_obj.changeNameByLookup(
                "Beelzebub",
                self.get_checks_boss_name("Beelzebub", world, immersive=True),
            )
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e749"], BytesIO(bytes(e749_obj.toBytes()))
        )

        # Beelzebub callback is 0x2e8 don't forget it
        f044_obj = self.get_script_obj_by_name("f044")

        f044_belz_rwms = f044_obj.appendMessage(
            self.get_reward_str("Beelzebub", world), "BELZ_RWMS"
        )
        ...
        f044_obj.changeMessageByIndex(
            assembler.message(
                "> ^r"
                + self.get_checks_boss_name("Beelzebub", world)
                + "^p can be felt^nbehind the door.^n^x> Will you enter?",
                "F044_DOOR03",
            ),
            0x83,
        )
        ...
        f044_kalpa3_tunnel_insts, f044_kalpa3_tunnel_labels = (
            f044_obj.getProcInstructionsLabelsByIndex(f044_kalpa3_tunnel_proc)
        )
        ...
        f044_kalpa3_tunnel_insts = (
            f044_kalpa3_tunnel_insts[0:60]
            + f044_kalpa3_tunnel_insert_insts
            + f044_kalpa3_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f044_kalpa3_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f044_obj.changeProcByIndex(
            f044_kalpa3_tunnel_insts, f044_kalpa3_tunnel_labels, f044_kalpa3_tunnel_proc
        )

        f044_lobby_tunnel_proc = f044_obj.getProcIndexByLabel("001_01eve_03")
        f044_lobby_tunnel_insts, f044_lobby_tunnel_labels = (
            f044_obj.getProcInstructionsLabelsByIndex(f044_lobby_tunnel_proc)
        )
        ...
        f044_lobby_tunnel_insts = (
            f044_lobby_tunnel_insts[0:60]
            + f044_lobby_tunnel_insert_insts
            + f044_lobby_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f044_lobby_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f044_obj.changeProcByIndex(
            f044_lobby_tunnel_insts, f044_lobby_tunnel_labels, f044_lobby_tunnel_proc
        )

        f044_kalpa5_tunnel_proc = f044_obj.getProcIndexByLabel("002_01eve_02")
        f044_kalpa5_tunnel_insts, f044_kalpa5_tunnel_labels = (
            f044_obj.getProcInstructionsLabelsByIndex(f044_kalpa5_tunnel_proc)
        )
        ...
        f044_kalpa5_tunnel_insts = (
            f044_kalpa5_tunnel_insts[0:60]
            + f044_kalpa5_tunnel_insert_insts
            + f044_kalpa5_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f044_kalpa5_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f044_obj.changeProcByIndex(
            f044_kalpa5_tunnel_insts, f044_kalpa5_tunnel_labels, f044_kalpa5_tunnel_proc
        )

        f044_star_tunnel_proc = f044_obj.getProcIndexByLabel("030_01eve_01")
        f044_star_tunnel_insts, f044_star_tunnel_labels = (
            f044_obj.getProcInstructionsLabelsByIndex(f044_star_tunnel_proc)
        )
        ...
        f044_star_tunnel_insts = (
            f044_star_tunnel_insts[0:60]
            + f044_star_tunnel_insert_insts
            + f044_star_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f044_star_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f044_obj.changeProcByIndex(
            f044_star_tunnel_insts, f044_star_tunnel_labels, f044_star_tunnel_proc
        )
        ...
        if config_settings.menorah_groups:  # Menorah hint
            f044_obj.changeMessageByIndex(
                assembler.message(
                    "> Retrieve these candelabra^nfrom ^g"
                    + self.get_flag_reward_location_string(0x3E1, world)
                    + "^p.",
                    "MSG_001_2",
                ),
                0x7,
        ...
        f044_door_insts, f044_door_labels = f044_obj.getProcInstructionsLabelsByIndex(
            f044_door_proc
        )
        precut = 14
        postcut = 40
        ...
        f044_switch_insts, f044_switch_labels = (
            f044_obj.getProcInstructionsLabelsByIndex(f044_switch_proc)
        )
        precut = 108
        postcut = 137
        diff = postcut - precut
        ...
        f044_obj.changeProcByIndex(
            f044_switch_insts, f044_switch_labels, f044_switch_proc
        )

        f044_menorah_proc = f044_obj.getProcIndexByLabel(
            "001_01eve_01"
        )  # Shorten placing menorahs
        f044_menorah_insts, f044_menorah_labels = (
            f044_obj.getProcInstructionsLabelsByIndex(f044_menorah_proc)
        )
        ...
        f044_menorah_insts = (
            f044_menorah_insts[:precut]
            + f044_menorah_insts[postcut:precut2]
            + f044_menorah_insts[postcut2:precut3]
            + f044_menorah_insts[postcut3:precut4]
            + f044_menorah_insts[postcut4:precut5]
            + f044_menorah_insts[postcut5:precut6]
            + f044_menorah_insts[postcut6:precut7]
            + f044_menorah_insts[postcut7:]
        )
        f044_obj.changeProcByIndex(
            f044_menorah_insts, f044_menorah_labels, f044_menorah_proc
        )
        ...
        e730_main_insts, e730_main_labels = e730_obj.getProcInstructionsLabelsByIndex(
            e730_main_proc
        )
        precut = 231
        postcut = 1060
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e730"], BytesIO(bytes(e730_obj.toBytes()))
        )

        # Shorten Metatron
        e750_obj = self.get_script_obj_by_name("e750")
        e750_main_proc = e750_obj.getProcIndexByLabel("e750_main")
        e750_main_insts, e750_main_labels = e750_obj.getProcInstructionsLabelsByIndex(
            e750_main_proc
        )
        if not config_settings.longest_cutscenes:
            ...
            e750_main_labels = [assembler.label("META_FOUGHT", 15)]
            e750_obj.changeProcByIndex(
                e750_main_insts, e750_main_labels, e750_main_proc
            )
        else:  # Metatron model swap
            e750_10_proc = e750_obj.getProcIndexByLabel("e750_010")
            e750_10_insts, e750_10_labels = e750_obj.getProcInstructionsLabelsByIndex(
                e750_10_proc
            )
            ...
            e750_main_insts[62] = inst(
                "PUSHIS", self.get_checks_boss_id("Metatron", world)
            )
            ...
            e750_meta_message = assembler.message(
                "Listen! And tremble in fear!^n^xI am "
                + self.get_checks_boss_name("Metatron", world, immersive=True)
                + "! I am one with god!^n^x...By his will, I shall destroy you!!",
                "MSG_004",
            )
            e750_meta_message.name_id = e750_meta_name_id
            e750_obj.changeMessageByIndex(e750_meta_message, 0x3)
            e750_obj.changeNameByLookup(
                "Metatron", self.get_checks_boss_name("Metatron", world, immersive=True)
            )
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e750"], BytesIO(bytes(e750_obj.toBytes()))
        )
        ...
        f045_tutorial_end_insts, f045_tutorial_end_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_tutorial_end_proc)
        )
        ...
        f045_obj.changeProcByIndex(
            f045_tutorial_end_insts, f045_tutorial_end_labels, f045_tutorial_end_proc
        )

        f045_tutorial_start_proc = f045_obj.getProcIndexByLabel("030_voice")
        f045_tutorial_start_insts, f045_tutorial_start_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_tutorial_start_proc)
        )
        ...
        f045_obj.changeProcByIndex(
            f045_tutorial_start_insts,
            f045_tutorial_start_labels,
            f045_tutorial_start_proc,
        )

        f045_meta_rwms = f045_obj.appendMessage(
            self.get_reward_str("Metatron", world), "META_RWMS"
        )
        ...
        f045_obj.changeMessageByIndex(
            assembler.message(
                "> You sense ^r"
                + self.get_checks_boss_name("Metatron", world)
                + "^p^nbeyond the door.^n^x> Will you enter?",
                "F045_DOOR02",
            ),
            0x98,
        ...
        f045_magic1_insts, f045_magic1_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_magic1_proc)
        )
        f045_magic1_insts[61] = inst("PUSHIS", 0xE)
        f045_obj.changeProcByIndex(
            f045_magic1_insts, f045_magic1_labels, f045_magic1_proc
        )
        f045_magic2_proc = f045_obj.getProcIndexByLabel("014_01eve_03")
        f045_magic2_insts, f045_magic2_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_magic2_proc)
        )
        f045_magic2_insts[61] = inst("PUSHIS", 0xE)
        f045_obj.changeProcByIndex(
            f045_magic2_insts, f045_magic2_labels, f045_magic2_proc
        )
        f045_agility1_proc = f045_obj.getProcIndexByLabel("014_01eve_01")
        f045_agility1_insts, f045_agility1_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_agility1_proc)
        )
        f045_agility1_insts[61] = inst("PUSHIS", 0xE)
        f045_obj.changeProcByIndex(
            f045_agility1_insts, f045_agility1_labels, f045_agility1_proc
        )
        f045_agility2_proc = f045_obj.getProcIndexByLabel("019_01eve_01")
        f045_agility2_insts, f045_agility2_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_agility2_proc)
        )
        f045_agility2_insts[61] = inst("PUSHIS", 0xE)
        f045_obj.changeProcByIndex(
            f045_agility2_insts, f045_agility2_labels, f045_agility2_proc
        )

        # Skip kalpa 5 tunnels
        f045_kalpa4_tunnel_proc = f045_obj.getProcIndexByLabel("001_01eve_02")
        f045_kalpa4_tunnel_insts, f045_kalpa4_tunnel_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_kalpa4_tunnel_proc)
        )
        ...
        f045_kalpa4_tunnel_insts = (
            f045_kalpa4_tunnel_insts[0:60]
            + f045_kalpa4_tunnel_insert_insts
            + f045_kalpa4_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f045_kalpa4_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f045_obj.changeProcByIndex(
            f045_kalpa4_tunnel_insts, f045_kalpa4_tunnel_labels, f045_kalpa4_tunnel_proc
        )

        f045_lobby_tunnel_proc = f045_obj.getProcIndexByLabel("001_01eve_03")
        f045_lobby_tunnel_insts, f045_lobby_tunnel_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_lobby_tunnel_proc)
        )
        ...
        f045_lobby_tunnel_insts = (
            f045_lobby_tunnel_insts[0:60]
            + f045_lobby_tunnel_insert_insts
            + f045_lobby_tunnel_insts[62:-1]
            + [inst("END")]
        )
        for l in f045_lobby_tunnel_labels:
            if l.label_offset > 60:
                l.label_offset += 4
        f045_obj.changeProcByIndex(
            f045_lobby_tunnel_insts, f045_lobby_tunnel_labels, f045_lobby_tunnel_proc
        )

        f045_menorah_proc = f045_obj.getProcIndexByLabel(
            "001_01eve_01"
        )  # Shorten placing menorah
        f045_menorah_insts, f045_menorah_labels = (
            f045_obj.getProcInstructionsLabelsByIndex(f045_menorah_proc)
        )
                ...
                if l.label_offset > precut2:
                    ...
                    if l.label_offset > precut3:
                        ...
                        if l.label_offset > precut4:
                            l.label_offset -= diff4
        f045_menorah_insts = (
            f045_menorah_insts[:precut]
            + f045_menorah_insts[postcut:precut2]
            + f045_menorah_insts[postcut2:precut3]
            + f045_menorah_insts[postcut3:precut4]
            + f045_menorah_insts[postcut4:]
        )
        f045_obj.changeProcByIndex(
            f045_menorah_insts, f045_menorah_labels, f045_menorah_proc
        )

        f045_demon_name_id = (
            f045_obj.sections[3].messages[0xAE].name_id
        )  # Taotie dialogue in Kalpa 5
        f045_demon_message = assembler.message(
            "CAN YOU FEEL IT?^n^xI CAN FEEL GREAT POWER^nFROM THIS DOOR,^nWHICH LEADS BELOW...^xSOMETHING EVEN STRONGER^nTHAN "
            + self.get_checks_boss_name("Beelzebub", world, immersive=True).upper()
            + "...^n^xIF YOU CHOOSE TO PROCEED,^nBE CAREFUL.",
            "MSG_F45_A18_AKUMA01",
        )
        ...
        e718_main_insts, e718_main_labels = e718_obj.getProcInstructionsLabelsByIndex(
            e718_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e718"], BytesIO(bytes(e718_obj.toBytes()))
        )

        e719_obj = self.get_script_obj_by_name("e719")
        e719_main_proc = e719_obj.getProcIndexByLabel("e719_main")
        e719_main_insts, e719_main_labels = e719_obj.getProcInstructionsLabelsByIndex(
            e719_main_proc
        )
        ...
        self.dds3.add_new_file(
            custom_vals.SCRIPT_OBJ_PATH["e719"], BytesIO(bytes(e719_obj.toBytes()))
        )
...
if __name__ == "__main__":
    ...
    with open("rom/DDS3.DDT", "rb") as ddt, open("rom/DDS3.IMG", "rb") as img:
        iso.export_iso(
            "rom/modified_scripts.iso", {"DDS3.DDT;1": ddt, "DDS3.IMG;1": img}
        )
