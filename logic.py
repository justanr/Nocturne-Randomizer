...
def init_magatamas(world):
    ...
    world.add_magatama(
        "Muspell", [], 15
    )  # Muspell is not good enough for ailment based bosses
...
def create_areas(world, config_settings):
    ...
    if config_settings.menorah_groups:
        world.add_flag(
            "Kalpa 2 Menorahs", 0x3E7, additional_ids=[0x3E8]
        )  # 3e7 is Daisojou, 3e8 is Hell Biker
        world.add_flag(
            "Kalpa 3 Menorahs", 0x3E4, additional_ids=[0x3E3, 0x3E2]
        )  # 3e4 is White Rider, 3e3 is Red Rider, 3e2 is Black Rider
        world.add_flag(
            "Kalpa 4 Menorahs", 0x3E1, additional_ids=[0x3E6, 0x3E5, 0x3EB]
        )  # 3e1 is Pale Rider, 3e6 is Harlot, 3e5 is Trumpeter, 3eb is Dante 2
...
def find_progressive_reward(state, check_pool, reward_pool, config_settings):
    for r in reward_pool:
        state.get_reward(r)
        completeable_checks = [
            c for c in check_pool if c.can_reach(state) and c.boss.can_beat(state)
        ]
        can_progress = bool(completeable_checks)
        if can_progress or (
            r.name in ["Kalpa 2 Menorahs"] and state.has_checked("Matador")
        ):
            return r
        else:
...
def randomize_world(world, logger, config_settings, attempts=100):
    ...
    while world.get_check("Kagutsuchi") in check_pool:
            ...
            for check in check_pool:
                if check.can_reach(state) and check in check_pool:
                    if check.boss.can_beat(state):
                        ...
                        logger.info(
                            "Beating " + check.boss.name + " at check: " + check.name
                        )
        ...
        can_progress = False
        shuffled_bosses = copy.copy(
            [
                b
                for b in bosses_progressed
                if b.check.area.name not in no_progression_areas
            ]
        )
        random.shuffle(shuffled_bosses)
        chosen_reward = find_progressive_reward(
            state, check_pool, reward_pool, config_settings
        )
        ...
        no_reward_boss_pool = [
            ...
            if b.reward == None and b.check.flag_rewards == []
        ]
    ...
    while reward_pool:
        ...
        no_reward_boss_pool = [
            b
            for b in bosses_progressed
            if b.check.area.name != "ToK"
            and b.reward == None
            and b.check.flag_rewards == []
        ]
        if no_reward_boss_pool != []:
...
if __name__ == "__main__":
    ...
    while world == None:
        ...
        world = randomize_world(
            world, logger, Settings()
        )  # These lines should never get run I think
